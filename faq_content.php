...
function local_rtocompliance_faq_data(): array {
    return [
        ...
        ['cat' => 'Issuing certificates', 'icon' => 'certificate', 'items' => [
            ...
            ['q' => 'How much does a certificate cost?', 'a' => 'Every certificate costs 5 credits (about A$0.50). You are shown the total cost before you generate, and nothing is issued for free — issuing from the queue is charged the same way.', 'page' => 'qual_cert_hub.php'],
            ['q' => 'What is the Qualification Certificate Hub?', 'a' => 'A dashboard showing, per qualification and variant, how many students are enrolled, complete, issued, pending, or queued for issue — with one-click actions to issue them.', 'page' => 'qual_cert_hub.php'],
            ['q' => 'What does the "Queue" column mean?', 'a' => 'Completed students lined up for certificate issue. Nothing issues them on its own — there is no scheduled task that processes the queue. Click Process Queue on the qualification\'s Detail page, or re-run a generation page.', 'page' => 'qual_cert_hub.php'],
            ...
            ['q' => 'I generated a certificate and paid, but nothing is in the register — where did it go?', 'a' => 'Nowhere: it was never created, and you were not charged. Every pre-issue check runs BEFORE credits are consumed, so a refused certificate is free. The confirmation dialog quotes a cost before the server is contacted, so seeing "this will charge 10 credits" is not proof that any were spent — check your balance in the sidebar. The reason for the refusal is shown on the generation page in the USI column and in the summary banner after the page reloads.', 'page' => 'generate_qual_certs.php'],
            ['q' => 'Why is a student greyed out and impossible to tick?', 'a' => 'Because the certificate would be refused, so the page will not let you pay for it. The USI column on that row gives the exact reason — no USI recorded, a USI that has not been verified with the USI Registry, no student record, or missing RTO details. Click "Add / verify USI" on the row to open that student\'s profile and fix it; once the USI is verified the tick box becomes available.', 'page' => 'generate_qual_certs.php'],
            ['q' => 'What does the USI column on the generation pages mean?', 'a' => 'Verified = issuable. Not verified = a USI is stored but the Registry has not confirmed it, which is not enough. Missing = no USI at all. No student record = the person has no RTO Compliance record. Duplicate student record = the person has more than one record so their USI cannot be resolved reliably. Not required = the course only warrants a Completion Certificate, which is not AQF certification and needs no USI.', 'page' => 'generate_course_certs.php'],
            ['q' => 'A student now has their USI verified — do I have to re-run the generation?', 'a' => 'Yes, someone has to start it. The student stays in the certificate queue as pending rather than being wrongly closed off, but nothing issues it on its own — there is no scheduled task that processes the queue. Either re-run the generation page, or open that qualification\'s Detail page in the Qualification Certificate Hub and use Process Queue. Remember certification is due within 30 days of the competent assessment.', 'page' => 'qual_cert_hub.php'],
            ['q' => 'Nobody at all can be issued a certificate — what is wrong?', 'a' => 'Check your RTO details first. A Testamur must carry the RTO legal name, national provider code and authorised signatory; while any of those are unset EVERY nationally recognised certificate is refused, for every student. It looks identical to a USI problem from the outside. Set them in RTO Settings.', 'page' => 'plugin_settings.php'],
        ]],
        ...
        ['cat' => 'Credits & billing', 'icon' => 'coin', 'items' => [
            ...
            ['q' => 'Are certificates issued from the queue free?', 'a' => 'No. Issuing from the queue is charged the same 5 credits per certificate as manual issue — nothing is issued for free.', 'page' => 'qual_cert_hub.php'],
