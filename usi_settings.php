...
if ($usiaction === 'conntest' && confirm_sesskey()) {
    ...
    if ($conntestok) {
        ...
        // E2003-DIAG (v6.3.0, 13 Aug 2026): ROOT CAUSE FOUND AND CORRECTED.
        //
        // E2003 "relying party in AppliesTo not recognised" was NEVER an ATO Access Manager
        // authorisation problem, and was NEVER a wrong URL. Both of those diagnoses (v6.2.61
        // and v6.2.65) were wrong and are retracted. Evidence:
        //   * There is NO mechanism in ATO Access Manager to authorise a machine credential
        //     for the USI Registry. RAM grants full permissions on creation; the ATO's
        //     relying-party list is global, not per-credential. IFCBAA called the ATO and
        //     were told the request made no sense.
        //   * The URL was correct all along. It matches the wsa10:Address published in the
        //     live production WSDL exactly. 4,248 logged attempts across BOTH capitalisations
        //     of that URL returned E2003 — proving casing was irrelevant.
        //
        // ACTUAL CAUSE: the platform's RequestSecurityToken did not match the
        // <sp:RequestSecurityTokenTemplate> published in the production WSDL. It requested a
        // Bearer/SAML2.0 token with no KeySize, no KeyWrapAlgorithm, no Claims and no
        // algorithm values. The ATO's Vanguard STS resolves a relying party TOGETHER WITH the
        // requested token shape, so an unregistered shape is reported as an unrecognised
        // relying party. Corrected to SymmetricKey / KeySize 256 / rsa-oaep-mgf1p / the two
        // Vanguard claims (abn, credentialtype) / aes256-cbc / hmac-sha1 — after which the
        // ATO issued a token.
        //
        // This is a PLATFORM fix (lms-labs.com). Nothing here, and nothing the RTO can do at
        // the ATO, affects it. Never send an RTO to Access Manager for E2003.
        $ise2003 = (stripos($tmsg, 'E2003') !== false || stripos($lc, 'relying party') !== false
                    || stripos($lc, 'appliesto') !== false);
        if ($ise2003) {
            $conntestmsg = 'USI connection test FAILED with ATO error E2003 — "the relying party specified in '
                . 'the AppliesTo element is not recognised"'
                . ($detail !== '' ? ' — the ATO said: "' . s($detail) . '"' : '') . '. '
                . 'THIS IS NOT SOMETHING YOU CAN FIX. It is a fault in the lms-labs.com platform\'s security-token '
                . 'request, not in Moodle, not in your machine credential, and not in your ATO or RAM setup. '
                . 'Do NOT contact the ATO or RAM about this error, and do not re-upload your credential — neither '
                . 'will change it. Please report this error to lms-labs.com support, quoting the date and time '
                . 'above. No student data was changed.';
        } else if ($isatosts) {
            $conntestmsg = 'USI connection test FAILED at the ATO security-token step (status "'
                . s($status ?: 'ATO_ERROR') . '"). The platform reached the ATO Machine Authentication Service '
                . '(softwareauthorisations.ato.gov.au) but the request was rejected'
                . ($detail !== '' ? ' — the ATO said: "' . s($detail) . '"' : '') . '. '
                . 'The most common causes are handled by lms-labs.com, not by you: the stored credential has '
                . 'expired or been revoked, or the token request does not match what the ATO expects. '
                . 'One thing worth checking on your side: that the machine credential held for your TOID has not '
                . 'expired and has not been revoked in RAM. Otherwise please report this error to lms-labs.com '
                . 'support with the date and time above. No student data was changed.';
        } else {
...
if ($usiaction === 'uploadcert' && confirm_sesskey()) {
    ...
    $orgid = trim((string) optional_param('certorgid', '', PARAM_RAW)); // pipeline-ignore: PARAM_RAW — free-text payload; sanitised/validated immediately after read, never echoed raw.
    if ($orgid === '') {
        $orgid = (string) (get_config('local_rtocompliance', 'usi_organization_id')
            ?: get_config('local_rtocompliance', 'rtocode') ?: '');
    }
    $certpass    = (string) optional_param('certpass', '', PARAM_RAW); // pipeline-ignore: PARAM_RAW — free-text payload; sanitised/validated immediately after read, never echoed raw.
    $certtestmode = (optional_param('certenv', 'prod', PARAM_ALPHA) === 'test');
    $notifemail  = trim((string) optional_param('certemail', '', PARAM_RAW)); // pipeline-ignore: PARAM_RAW — free-text payload; sanitised/validated immediately after read, never echoed raw.
...
$cards = [
    ...
    // STATS-VISIBILITY FIX (v6.3.0): pending_retry was computed into $npending but never
    // rendered, so every student parked on a transient error was invisible. On one live site
    // that hid 1,047 of 1,060 students holding a USI. The cards above and below must account
    // for every student with a USI.
    ['Awaiting retry',      $npending,   '#8b5cf6', 'unverified'],
