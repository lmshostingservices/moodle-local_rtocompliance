...
function local_rtocompliance_support_faq_data(): array {
    ...
    [
        ...
        'faqs'     => [
            ...
            [
                'question' => 'How does the certificate queue (autocert) work?',
                'answer'   => 'Every time a Moodle course completion event fires, the enrolment reconciler checks whether the student now has Competent outcomes (C, RPL, or CT) on every unit in the qualification — across the primary linked course and all variant courses. If all units are complete it sets the student\'s programme outcome to Complete, records the AVETMISS result code, and QUEUES them for their Testamur. The queueing is automatic; the issuing is not. No scheduled task issues certificates — open the qualification\'s Detail page in the Qualification Certificate Hub and click Process Queue, or run a generation page. This matters, because the 30-day issuance clock in Compliance Requirements Clause 9(2) begins from the date the final course completion fires, and a queue nobody processes does not stop that clock.',
            ],
            [
