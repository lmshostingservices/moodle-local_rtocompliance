...
class cert_template {
    ...
    public static function get_dynamic_field_catalogue(): array {
        ...
        return [
            ...
            'student.usi'                            => ['label' => 'Unique Student Identifier (USI)',         'group' => 'Student',       'required_for' => ['record'],                                        'sample' => 'AB12CD34EF', 'forbidden_for' => ['testamur', 'statement']],
            ...
            'qualification.units'                    => ['label' => 'Units of competency (flat list — SOA/legacy)',  'group' => 'Qualification', 'required_for' => ['statement', 'record'],                           'sample' => 'BSBCMM311 — Apply critical thinking skills…'],
            // ROR-3COL-FIX (v5.9.220): 3-column keys for Record of Results.
            // Use these instead of qualification.units to get Semester/Year, Unit, and Result
            // in separate positioned columns.  They resolve from the same unit JSON but each
            // returns only its own column as a multi-line string (one line per unit).
            'qualification.units_col_semester'       => ['label' => 'RoR Col 1 — Semester / Year',              'group' => 'Qualification', 'required_for' => [],                                                'sample' => "Sem 1 2024\nSem 2 2024"],
            'qualification.units_col_names'          => ['label' => 'RoR Col 2 — Unit codes & names',           'group' => 'Qualification', 'required_for' => [],                                                'sample' => "BSBCMM311 — Apply critical thinking skills…"],
            'qualification.units_col_results'        => ['label' => 'RoR Col 3 — Results',                      'group' => 'Qualification', 'required_for' => [],                                                'sample' => "Competent\nCompetent"],
            ...
            'rto.code'                               => ['label' => 'RTO code (TOID/RTO ID)',                  'group' => 'RTO',           'required_for' => ['testamur', 'statement', 'record'],               'sample' => '50918'],
            ...
            'signatory.title'                        => ['label' => 'Signatory position/title',                'group' => 'Signatory',     'required_for' => ['testamur', 'statement'],                         'sample' => 'Chief Executive Officer'],
            'signatory.signature'                    => ['label' => 'Signatory signature (from Branding panel)', 'group' => 'Signatory',   'required_for' => ['testamur', 'statement'],                         'sample' => '[CEO signature image]'],
            ...
            'organisation_seal'                      => ['label' => 'Organisation seal / corporate identifier / watermark', 'group' => 'Compliance', 'required_for' => ['testamur', 'statement'],                'sample' => '[Org seal]'],
    ...
    public static function build_starter_design(string $certtype, ?string $orientation = null): array {
        ...
        switch ($certtype) {
            case 'testamur':   return $o === 'P' ? self::starter_testamur_portrait()   : self::starter_testamur();
            case 'statement':  return $o === 'L' ? self::starter_statement_landscape() : self::starter_statement();
            case 'record':     return $o === 'L' ? self::starter_record_landscape()    : self::starter_record();
            case 'completion': return $o === 'P' ? self::starter_completion_portrait() : self::starter_completion();
        }
        return self::blank_page('L');
    }
    ...
    private static function starter_testamur(): array {
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure', 'x_mm' => 197, 'y_mm' => 200, 'w_mm' => 80, 'h_mm' => 5, 'fontsize' => 8, 'align' => 'R', 'color' => '#888888']);
    ...
    private static function starter_statement(): array {
        ...
        // "Form part of" + qual (renderer auto-builds when SoA is part of a qual).
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'qualification.partofstatement','x_mm' => 15, 'y_mm' => 192, 'w_mm' => 180, 'h_mm' => 6, 'fontsize' => 10, 'fontstyle' => 'I', 'align' => 'C']);
        // v4.2.61 — Optional descriptors per fact sheet page 4. Render blank when unused.
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'qualification.completionofcoursestatement', 'x_mm' => 15, 'y_mm' => 199, 'w_mm' => 180, 'h_mm' => 5, 'fontsize' => 9, 'fontstyle' => 'I', 'align' => 'C', 'color' => '#444444']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'skill_set_statement',         'x_mm' => 15, 'y_mm' => 204, 'w_mm' => 180, 'h_mm' => 5, 'fontsize' => 9, 'fontstyle' => 'I', 'align' => 'C', 'color' => '#444444']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'language_statement',          'x_mm' => 15, 'y_mm' => 209, 'w_mm' => 180, 'h_mm' => 5, 'fontsize' => 9, 'fontstyle' => 'I', 'align' => 'C', 'color' => '#444444']);
        // Signature.
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure',        'x_mm' => 110, 'y_mm' => 250, 'w_mm' => 85, 'h_mm' => 5, 'fontsize' => 8, 'align' => 'R', 'color' => '#888888']);
    ...
    private static function starter_record(): array {
        ...
        // ASQA-COMPLIANCE-PASS-2 (v4.2.59) — fact sheet labels.
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 15, 'y_mm' => 65, 'w_mm' => 45, 'h_mm' => 6, 'fontsize' => 10, 'fontstyle' => 'B', 'text' => 'Name of student:']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'student.fullname',          'x_mm' => 60, 'y_mm' => 65, 'w_mm' => 135, 'h_mm' => 6, 'fontsize' => 12, 'fontstyle' => 'B']);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 15, 'y_mm' => 73, 'w_mm' => 45, 'h_mm' => 6, 'fontsize' => 10, 'fontstyle' => 'B', 'text' => 'USI:']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'student.usi',               'x_mm' => 60, 'y_mm' => 73, 'w_mm' => 135, 'h_mm' => 6, 'fontsize' => 11, 'font' => 'courier']);
        // Qualification line — REQUIRED per fact sheet (was missing in v4.2.58).
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 15, 'y_mm' => 81, 'w_mm' => 45, 'h_mm' => 6, 'fontsize' => 10, 'fontstyle' => 'B', 'text' => 'Name of qualification:']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'qualification.code',        'x_mm' => 60, 'y_mm' => 81, 'w_mm' => 25, 'h_mm' => 6, 'fontsize' => 11, 'fontstyle' => 'B']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'qualification.name',        'x_mm' => 86, 'y_mm' => 81, 'w_mm' => 109, 'h_mm' => 6, 'fontsize' => 11]);
        // Units table — column headers per fact sheet (Semester/Year | Units enrolled | Results).
        $fields[] = self::mkf($id, 'line',    ['x_mm' => 15, 'y_mm' => 92, 'w_mm' => 180, 'h_mm' => 0, 'linewidth' => 0.3]);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 15, 'y_mm' => 93, 'w_mm' => 30, 'h_mm' => 6, 'fontsize' => 9, 'fontstyle' => 'B', 'text' => 'Semester / Year']);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 47, 'y_mm' => 93, 'w_mm' => 110, 'h_mm' => 6, 'fontsize' => 9, 'fontstyle' => 'B', 'text' => 'Units / modules enrolled']);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 159, 'y_mm' => 93, 'w_mm' => 36, 'h_mm' => 6, 'fontsize' => 9, 'fontstyle' => 'B', 'text' => 'Results']);
        $fields[] = self::mkf($id, 'line',    ['x_mm' => 15, 'y_mm' => 100, 'w_mm' => 180, 'h_mm' => 0, 'linewidth' => 0.3]);
        // ROR-TABLE-FIX (v5.9.246): single ror_table field renders all three columns
        // row-by-row, measuring each row's required height before advancing Y so
        // all columns stay in vertical sync even when unit names wrap to 2+ lines.
        // Replaces the 3 independent fixed-height MultiCell fields that caused
        // col1/col3 to clip after ~11 units whenever col2 had long wrapping names.
        // col1_w=30 (semester), col2_w=110 (names), col3_w=36 (results); 2mm gaps.
        $fields[] = self::mkf($id, 'ror_table', ['x_mm' => 15, 'y_mm' => 102, 'w_mm' => 180, 'h_mm' => 115, 'fontsize' => 10, 'col1_w' => 30, 'col2_w' => 110, 'col3_w' => 36]);
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure',      'x_mm' => 15,  'y_mm' => 264, 'w_mm' => 180, 'h_mm' => 8, 'fontsize' => 8, 'align' => 'C', 'color' => '#444444']);
    ...
    private static function starter_statement_landscape(): array {
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'qualification.partofstatement',  'x_mm' => 30,  'y_mm' => 159, 'w_mm' => 237, 'h_mm' => 5, 'fontsize' => 10, 'fontstyle' => 'I', 'align' => 'C']);
        // v4.2.61 — Optional descriptors per fact sheet page 4. Render blank when unused.
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'qualification.completionofcoursestatement', 'x_mm' => 30, 'y_mm' => 165, 'w_mm' => 237, 'h_mm' => 4, 'fontsize' => 9, 'fontstyle' => 'I', 'align' => 'C', 'color' => '#444444']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'skill_set_statement',         'x_mm' => 30, 'y_mm' => 169, 'w_mm' => 237, 'h_mm' => 4, 'fontsize' => 9, 'fontstyle' => 'I', 'align' => 'C', 'color' => '#444444']);
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'language_statement',          'x_mm' => 30, 'y_mm' => 173, 'w_mm' => 237, 'h_mm' => 4, 'fontsize' => 9, 'fontstyle' => 'I', 'align' => 'C', 'color' => '#444444']);
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure','x_mm' => 200, 'y_mm' => 205, 'w_mm' => 82, 'h_mm' => 4, 'fontsize' => 8, 'align' => 'R', 'color' => '#888888']);
    ...
    private static function starter_record_landscape(): array {
        ...
        // Units table — fact sheet column headers.
        $fields[] = self::mkf($id, 'line',    ['x_mm' => 15, 'y_mm' => 82, 'w_mm' => 267, 'h_mm' => 0, 'linewidth' => 0.3]);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 15, 'y_mm' => 83, 'w_mm' => 40, 'h_mm' => 6, 'fontsize' => 9, 'fontstyle' => 'B', 'text' => 'Semester / Year']);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 57, 'y_mm' => 83, 'w_mm' => 175, 'h_mm' => 6, 'fontsize' => 9, 'fontstyle' => 'B', 'text' => 'Units / modules enrolled']);
        $fields[] = self::mkf($id, 'text',    ['x_mm' => 234, 'y_mm' => 83, 'w_mm' => 48, 'h_mm' => 6, 'fontsize' => 9, 'fontstyle' => 'B', 'text' => 'Results']);
        $fields[] = self::mkf($id, 'line',    ['x_mm' => 15, 'y_mm' => 90, 'w_mm' => 267, 'h_mm' => 0, 'linewidth' => 0.3]);
        // ROR-TABLE-FIX (v5.9.246): single ror_table field — row-by-row rendering.
        // Landscape: col1_w=40 (semester), col2_w=175 (names), col3_w=48 (results); 2mm gaps.
        $fields[] = self::mkf($id, 'ror_table', ['x_mm' => 15, 'y_mm' => 92, 'w_mm' => 267, 'h_mm' => 76, 'fontsize' => 10, 'col1_w' => 40, 'col2_w' => 175, 'col3_w' => 48]);
        // v4.2.61 — Optional language statement.
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure',      'x_mm' => 15,  'y_mm' => 200, 'w_mm' => 267, 'h_mm' => 5, 'fontsize' => 8, 'align' => 'C', 'color' => '#444444']);
    ...
    private static function starter_completion_portrait(): array {
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure', 'x_mm' => 15, 'y_mm' => 255, 'w_mm' => 180, 'h_mm' => 7, 'fontsize' => 8, 'align' => 'C', 'color' => '#666666']);
    ...
    private static function starter_completion(): array {
        ...
        $fields[] = self::mkf($id, 'dynamic', ['dynamickey' => 'authenticity_measure', 'x_mm' => 30, 'y_mm' => 196, 'w_mm' => 237, 'h_mm' => 5, 'fontsize' => 8, 'align' => 'C', 'color' => '#666666']);
    ...
    public static function get_branding_logo_url(): ?string {
        return self::get_branding_url(self::BRANDING_ITEMID_LOGO);
    }
    ...
    public static function get_branding_signature_url(): ?string {
        return self::get_branding_url(self::BRANDING_ITEMID_SIGNATURE);
    }
    ...
    public static function get_branding_org_seal_url(): ?string {
        return self::get_branding_url(self::BRANDING_ITEMID_ORG_SEAL);
    }
    ...
    public static function create(string $certtype, string $name,
            string $audience = 'default', ?string $audiencelabel = null): int {
        global $DB, $USER;
        ...
        $record = (object) [
            ...
            'status'        => 'draft',
            'isactive'      => 0,
            'designjson'    => json_encode(self::get_default_design($certtype)),
            'bgitemid'      => null,
            'lastvalidation' => null,
            'createdby'     => $USER->id,
            'approvedby'    => null,
    ...
    public static function pick_for_cert(\stdClass $cert): ?\stdClass {
        ...
        if (!empty($cert->certtmplid)) {
            $r = $DB->get_record('local_rtocompliance_certtmpl', ['id' => (int) $cert->certtmplid]);
            if ($r) {
                return $r;
            }
        }
    ...
    public static function activate(int $id): bool {
        ...
        try {
            ...
            foreach ($currentlyActive as $_existing) {
                ...
                $_eDesign = self::decode_design($_existing);
                $_eOrientation = $_eDesign['page']['orientation'] ?? 'L';
                if ($_eOrientation === $targetOrientation) {
                    $DB->update_record('local_rtocompliance_certtmpl', (object) [
                        'id'           => (int) $_existing->id,
                        'isactive'     => 0,
                        'timemodified' => time(),
    ...
    public static function duplicate(int $id): int {
        ...
        return (int) $DB->insert_record('local_rtocompliance_certtmpl', $template);
    }
