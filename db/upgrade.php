...
/**
 * Reconcile the live database against db/install.xml.
 *
 * WHY THIS EXISTS (v6.3.0)
 * -----------------------
 * Until v6.3.0 the savepoints in this file were 13-digit numbers while
 * version.php declared a 10-digit version, so $oldversion never matched any
 * guard and EVERY upgrade re-ran all 792 steps. That was wasteful, but it had
 * one accidental benefit: the schema was continually re-asserted, because every
 * step is individually guarded with field_exists()/table_exists().
 *
 * v6.3.0 renumbers the savepoints correctly, so historical steps are now
 * genuinely skipped — which is right, but it means a column that never got
 * added on a particular site would stay missing forever. This function closes
 * that gap by walking install.xml (the definition a fresh install builds from)
 * and adding anything the live database is missing.
 *
 * It is deliberately conservative:
 *   - it only CREATES missing tables and ADDS missing columns
 *   - it never drops, renames, retypes or reorders anything
 *   - it skips a NOT NULL column with no default on a table that already has
 *     rows, because there is no safe value to backfill — that case is reported
 *     instead, so a human decides
 *
 * @param database_manager $dbman
 * @return array ['tables' => string[], 'fields' => string[], 'skipped' => string[]]
 */
function local_rtocompliance_upgrade_sync_schema($dbman): array {
    global $CFG, $DB;

    $result = ['tables' => [], 'fields' => [], 'skipped' => []];
    ...
    if (!$xmldbfile->fileExists() || !$xmldbfile->loadXMLStructure()) {
        $result['skipped'][] = 'install.xml could not be read — schema reconciliation skipped';
        return $result;
    }

    $structure = $xmldbfile->getStructure();
    if (!$structure) {
        $result['skipped'][] = 'install.xml has no readable structure — schema reconciliation skipped';
        return $result;
    }
    ...
    foreach ($structure->getTables() as $table) {
        ...
        if (!$dbman->table_exists($table)) {
            try {
                ...
                $result['skipped'][] = "table {$tablename}: " . $e->getMessage();
            }
            continue;
        }

        $hasrows = null; // Counted lazily, and only when a risky field turns up.
        foreach ($table->getFields() as $field) {
            if ($dbman->field_exists($table, $field)) {
                continue;
            }

            $notnull = ($field->getNotNull() == XMLDB_NOTNULL);
            $default = $field->getDefault();
            if ($notnull && ($default === null || $default === '')) {
                if ($hasrows === null) {
                    $hasrows = $DB->count_records($tablename) > 0;
                }
                if ($hasrows) {
                    $result['skipped'][] = $tablename . '.' . $field->getName()
                        . ' (NOT NULL with no default, and the table already has rows)';
                    continue;
                }
            }
            ...
            try {
                ...
                $result['fields'][] = $tablename . '.' . $field->getName();
            } catch (\Throwable $e) {
                $result['skipped'][] = $tablename . '.' . $field->getName() . ': ' . $e->getMessage();
            }
        }
    }
...
function xmldb_local_rtocompliance_upgrade($oldversion) {
    ...
    if ($oldversion < 2025120400) {
        ...
        upgrade_plugin_savepoint(true, 2025120400, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025120401) {
        ...
        upgrade_plugin_savepoint(true, 2025120401, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025120700) {
        ...
        upgrade_plugin_savepoint(true, 2025120700, 'local', 'rtocompliance');
    }

    if ($oldversion < 2025120701) {
        ...
        upgrade_plugin_savepoint(true, 2025120701, 'local', 'rtocompliance');
    }
    ...
    // Version 1.8.0: AVETMISS Edition 2.3 compliance + AI features.
    ...
    // Version 1.8.0 Phase 2: Create AI Survey Insights table.
    ...
    // Version 1.8.0 Phase 3: Create AI Compliance Alerts table.
    ...
    // Version 2.0.0: ASQA 2025 Practice Guide Compliance Tables
    ...
    if ($oldversion < 2025121001) {
        ...
        upgrade_plugin_savepoint(true, 2025121001, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121002) {
        ...
        upgrade_plugin_savepoint(true, 2025121002, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121005) {
        ...
        upgrade_plugin_savepoint(true, 2025121005, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121008) {
        ...
        upgrade_plugin_savepoint(true, 2025121008, 'local', 'rtocompliance');
    }

    // Version 2.4.4 - CSS scoping fix
    if ($oldversion < 2025121009) {
        ...
        upgrade_plugin_savepoint(true, 2025121009, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121010) {
        ...
        upgrade_plugin_savepoint(true, 2025121010, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121200) {
        ...
        upgrade_plugin_savepoint(true, 2025121200, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121600) {
        ...
        upgrade_plugin_savepoint(true, 2025121600, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2025121700) {
        ...
        upgrade_plugin_savepoint(true, 2025121700, 'local', 'rtocompliance');
    }

    // Version 3.7.32 - Comprehensive check for all missing student fields
    if ($oldversion < 2025122600) {
        ...
        upgrade_plugin_savepoint(true, 2025122600, 'local', 'rtocompliance');
    }

    // Version 3.7.38 - Add trainer currency activities table for multiple currency records per trainer
    ...
    // Version 3.7.41 - Add trainer vocational competency activities table for multiple competency records per trainer
    if ($oldversion < 2025123000) {
        ...
        upgrade_plugin_savepoint(true, 2025123000, 'local', 'rtocompliance');
    }

    // Version 3.7.49 - Fix credentialrole column size and add taeexpirydate
    ...
    if ($oldversion < 2026030401) {
        ...
        upgrade_plugin_savepoint(true, 2026030401, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030402) {
        $table = new xmldb_table('local_rtocompliance_tas_consult');
        ...
        upgrade_plugin_savepoint(true, 2026030402, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026030403) {
        ...
        upgrade_plugin_savepoint(true, 2026030403, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026030600) {
        // Version 3.7.65 — version bump. No schema changes.
        upgrade_plugin_savepoint(true, 2026030600, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030601) {
        // Version 3.7.66 — Moodle enrolment import feature. No schema changes.
        upgrade_plugin_savepoint(true, 2026030601, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030602) {
        // Version 3.7.67 — Plugin-wide "not connected" audit fixes. No schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026030602, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030603) {
        // Version 3.7.68 — Version bump to force Moodle upgrade detection. No schema changes.
        upgrade_plugin_savepoint(true, 2026030603, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030604) {
        // Version 3.7.69 — Full version bump to force Moodle DB recognition. No schema changes.
        upgrade_plugin_savepoint(true, 2026030604, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030605) {
        // Version 3.7.70 — ROOT CAUSE FIX: Test Data Generator (Test Students) added to the
        ...
        upgrade_plugin_savepoint(true, 2026030605, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026030606) {
        // Version 3.7.71 — Added configurable audit log retention setting (log_retentiondays,
        ...
        upgrade_plugin_savepoint(true, 2026030606, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032300) {
        // Version 3.7.73 — Fix audit log "Array to string conversion" warning when
    ...
    if ($oldversion < 2026032600) {
        // Version 3.7.75 — Fix local_rtocompliance_exports schema mismatch.
        ...
        upgrade_plugin_savepoint(true, 2026032600, 'local', 'rtocompliance');
    }

    // Version 3.7.76: BUG FIX — Scoped debug popup error interceptors to prevent site admin
    ...
    if ($oldversion < 2026032602) {
        // Version 3.7.77 — Add RPL/Credit Transfer, Risk Management, Roles & Responsibilities,
        ...
        upgrade_plugin_savepoint(true, 2026032602, 'local', 'rtocompliance');
    }

    // Version 3.7.78: BUG FIX — Removed debug error popup from before_footer hook.
    ...
    if ($oldversion < 2026032603) {
        upgrade_plugin_savepoint(true, 2026032603, 'local', 'rtocompliance');
    }

    // Version 3.7.79: BUGFIX — CSS :contains() selector removed, :root variables scoped to path class,
    ...
    // Version 3.7.92: NEW — Nominal hours auto-lookup via NCVER API in Qualification Builder.
    ...
    if ($oldversion < 2026032701) {
        upgrade_plugin_savepoint(true, 2026032701, 'local', 'rtocompliance');
    }


    // Version 3.7.93: BUGFIX — Create 4 tables missing from upgrade.php for existing installations.
    ...
    if ($oldversion < 2026032702) {
        ...
        upgrade_plugin_savepoint(true, 2026032702, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032703) {
        // Version 3.7.94 — Testing Engine added (testing.php + settings.php nav entry + lang strings).
        // No new DB tables required.
        upgrade_plugin_savepoint(true, 2026032703, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032704) {
        // Version 3.7.95 — BUGFIX: Testing Engine — fixed infra_caps, qual_table, comp_risk, trainer_credentials, nat_locations tests.
        // No new DB tables required.
        upgrade_plugin_savepoint(true, 2026032704, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032800) {
        // Version 3.7.96 — VERSION-BUMP: Routine release. Adds missing upgrade.php savepoint for v3.7.95.
        // No new DB tables required.
        upgrade_plugin_savepoint(true, 2026032800, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032801) {
        // Version 3.7.97 — BUGFIX: Fix false-positive RTO-USI-005 diagnostic (usi_pending test).
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026032801, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032802) {
        // Version 3.7.98 — FIX: Add apiurl to API settings page so admins can configure the
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026032802, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032803) {
        // Version 3.8.0 — FEATURE: Smart Qualification Builder.
        ...
        upgrade_plugin_savepoint(true, 2026032803, 'local', 'rtocompliance');
    }

    // Version 3.8.1 — SAVEPOINT FIX: This savepoint was absent from the v3.8.1 release ZIP,
    ...
    if ($oldversion < 2026032804) {
        upgrade_plugin_savepoint(true, 2026032804, 'local', 'rtocompliance');
    }

    // Version 3.8.2 — VERSION-BUMP: Jumps numeric version to 2026032800030 to guarantee this
    ...
    if ($oldversion < 2026032805) {
        upgrade_plugin_savepoint(true, 2026032805, 'local', 'rtocompliance');
    }

    // Version 3.8.3 — FIX: Tester-reported crashes and UI issues.
    ...
    // No DB schema change.
    if ($oldversion < 2026032806) {
        upgrade_plugin_savepoint(true, 2026032806, 'local', 'rtocompliance');
    }

    // Version 3.8.4 — FIX: TGA fetch "Class curl not found" error.
    ...
    // No DB schema change.
    if ($oldversion < 2026032807) {
        upgrade_plugin_savepoint(true, 2026032807, 'local', 'rtocompliance');
    }

    // Version 3.8.5 — FEATURE: Qualification Builder full UX overhaul.
    ...
    if ($oldversion < 2026032808) {
        upgrade_plugin_savepoint(true, 2026032808, 'local', 'rtocompliance');
    }

    // Version 3.8.6 — FEATURE: Testing Engine fix actions.
    ...
    // No DB schema change.
    if ($oldversion < 2026032809) {
        upgrade_plugin_savepoint(true, 2026032809, 'local', 'rtocompliance');
    }

    // Version 3.8.7 — FIX: trainer_credentials autofix honesty.
    ...
    if ($oldversion < 2026032810) {
        upgrade_plugin_savepoint(true, 2026032810, 'local', 'rtocompliance');
    }

    // Version 3.8.8 — FEATURE: Platform config push pipeline.
    ...
    if ($oldversion < 2026032811) {
        upgrade_plugin_savepoint(true, 2026032811, 'local', 'rtocompliance');
    }

    // Version 3.8.9 — FEATURE: USI verification proxied through lms-labs.com platform.
    ...
    // Version 3.8.10 — Re-release bump.
    ...
    if ($oldversion < 2026032901) {
        upgrade_plugin_savepoint(true, 2026032901, 'local', 'rtocompliance');
    }

    // Version 3.8.11 — FIX: Central Config integration for USI proxy.
    ...
    // No DB schema change.
    if ($oldversion < 2026032902) {
        upgrade_plugin_savepoint(true, 2026032902, 'local', 'rtocompliance');
    }

    // Version 3.8.12 — CLEAN BUMP: Re-release of v3.8.11 changes assembled in a single clean pass.
    ...
    if ($oldversion < 2026032903) {
        upgrade_plugin_savepoint(true, 2026032903, 'local', 'rtocompliance');
    }

    // Version 3.8.13 — FIX: qualbuilder_edit.php blank white screen on Moodle 4.0 – 4.2.
    ...
    if ($oldversion < 2026032904) {
        upgrade_plugin_savepoint(true, 2026032904, 'local', 'rtocompliance');
    }

    // Version 3.8.14 — BUMP: Consolidation release.
    ...
    // No DB schema change.
    if ($oldversion < 2026032905) {
        upgrade_plugin_savepoint(true, 2026032905, 'local', 'rtocompliance');
    }

    // Version 3.8.15 — AMD FIX: Extracted qualbuilder_edit.php inline JS into proper AMD module.
    // qualbuilder_edit.js added to amd/src + amd/build. Now called via js_call_amd().
    // No DB schema change.
    if ($oldversion < 2026032906) {
        upgrade_plugin_savepoint(true, 2026032906, 'local', 'rtocompliance');
    }

    // Version 3.8.16 — BUMP: Consolidation release. All 6 locations synced via master release process.
    // No DB schema change.
    if ($oldversion < 2026032907) {
        upgrade_plugin_savepoint(true, 2026032907, 'local', 'rtocompliance');
    }

    // Version 3.8.17 — Certificate credit deduction: 5 credits consumed per certificate issued.
    ...
    // No DB schema change.
    if ($oldversion < 2026032908) {
        upgrade_plugin_savepoint(true, 2026032908, 'local', 'rtocompliance');
    }

    // Version 3.8.18 — CRITICAL AMD FIX: ReferenceError: setup is not defined.
    ...
    // No DB schema change.
    if ($oldversion < 2026032909) {
        upgrade_plugin_savepoint(true, 2026032909, 'local', 'rtocompliance');
    }

    // Version 3.8.19: BUG FIX — Site admin primary/secondary navigation menus disappearing on
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032910) {
        upgrade_plugin_savepoint(true, 2026032910, 'local', 'rtocompliance');
    }

    // Version 3.8.20: BUMP — Consolidation release following master release process (Replit Hardened Edition).
    ...
    if ($oldversion < 2026032911) {
        upgrade_plugin_savepoint(true, 2026032911, 'local', 'rtocompliance');
    }

    // Version 3.8.21: NAV FIX — qualbuilder_edit.php JS init payload moved from js_call_amd() inline args
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032912) {
        upgrade_plugin_savepoint(true, 2026032912, 'local', 'rtocompliance');
    }

    // Version 3.8.22: AMD ASCII CLEAN — All non-ASCII characters (em dashes U+2014, box-drawing U+2500)
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032913) {
        upgrade_plugin_savepoint(true, 2026032913, 'local', 'rtocompliance');
    }

    // Version 3.8.23: CONSOLIDATION BUMP — No code or DB schema changes.
    ...
    if ($oldversion < 2026032914) {
        upgrade_plugin_savepoint(true, 2026032914, 'local', 'rtocompliance');
    }

    // Version 3.8.24: HTML-ENTITY CLEAN — All HTML numeric entities inside JS string literals in
    ...
    if ($oldversion < 2026032915) {
        upgrade_plugin_savepoint(true, 2026032915, 'local', 'rtocompliance');
    }

    // Version 3.8.25: REBUMP — Full master release process (Replit Hardened Edition).
    ...
    if ($oldversion < 2026032916) {
        upgrade_plugin_savepoint(true, 2026032916, 'local', 'rtocompliance');
    }

    // Version 3.8.26: ENCODING FIX FINAL — Replaced ALL corrupted UTF-8/Latin-1 byte sequences
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032917) {
        upgrade_plugin_savepoint(true, 2026032917, 'local', 'rtocompliance');
    }

    // Version 3.8.27: Full ChatGPT master release process bump. All reality checks passed:
    ...
    if ($oldversion < 2026032918) {
        upgrade_plugin_savepoint(true, 2026032918, 'local', 'rtocompliance');
    }

    // Version 3.8.28: ChatGPT-confirmed final fix for Moodle nav crash bug.
    ...
    //          No DB schema changes.
    if ($oldversion < 2026032919) {
        upgrade_plugin_savepoint(true, 2026032919, 'local', 'rtocompliance');
    }

    // Version 3.8.29: CRITICAL SYNTAX FIX — JS SyntaxError confirmed root cause of nav crash.
    ...
    if ($oldversion < 2026032920) {
        upgrade_plugin_savepoint(true, 2026032920, 'local', 'rtocompliance');
    }

    // Version 3.8.30: Qual Builder UX -- auto-link flow + compact course badges + QPR banner + bulk nominal hours.
    ...
    //   No DB schema changes.
    if ($oldversion < 2026032921) {
        upgrade_plugin_savepoint(true, 2026032921, 'local', 'rtocompliance');
    }

    // Version 3.8.31: Fix 5 critical AVETMISS enrolment/completion pipeline bugs + AVETMISS 2.3 code audit.
    ...
    //   No DB schema changes.
    if ($oldversion < 2026032922) {
        upgrade_plugin_savepoint(true, 2026032922, 'local', 'rtocompliance');
    }

    // Version 3.8.32: nat_generator.php full rewrite (schema-verified v3, all 10 NAT files).
    ...
    //   No DB schema changes.
    if ($oldversion < 2026032923) {
        upgrade_plugin_savepoint(true, 2026032923, 'local', 'rtocompliance');
    }

    // Version 3.8.33: 50-bug compliance audit -- no DB schema changes.
    ...
    if ($oldversion < 2026032924) {
        upgrade_plugin_savepoint(true, 2026032924, 'local', 'rtocompliance');
    }

    // Version 3.8.34 - Security audit pass 2: 4 additional bugs fixed.
    ...
    //   No DB schema changes.
    if ($oldversion < 2026032925) {
        upgrade_plugin_savepoint(true, 2026032925, 'local', 'rtocompliance');
    }

    // Version 3.8.35 - CSRF fix: qualbuilder_unit.php delete confirm path.
    ...
    if ($oldversion < 2026032926) {
        upgrade_plugin_savepoint(true, 2026032926, 'local', 'rtocompliance');
    }

    // Version 3.8.36 - CSRF sweep pass: 3 additional require_sesskey() fixes.
    ...
    //   No DB schema changes.
    if ($oldversion < 2026032927) {
        upgrade_plugin_savepoint(true, 2026032927, 'local', 'rtocompliance');
    }

    // Version 3.8.37 - Master release process validation pass.
    ...
    if ($oldversion < 2026032928) {
        upgrade_plugin_savepoint(true, 2026032928, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032929) {
        ...
        upgrade_plugin_savepoint(true, 2026032929, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026032930) {
        // Version 3.8.39: 8-bug fix pass — no DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026032930, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026033000) {
        // Version 3.8.40: 5-bug fix pass — no DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026033000, 'local', 'rtocompliance');
    }

    // Version 3.8.41: VERSION BUMP — No code changes. No DB schema changes.
    if ($oldversion < 2026033001) {
        upgrade_plugin_savepoint(true, 2026033001, 'local', 'rtocompliance');
    }

    // Version 3.8.42: BUG FIX — survey_send.php and classes/external.php user object handling.
    ...
    // Version 3.8.43: BUG FIX — Two tester-reported bugs fixed.
    ...
    if ($oldversion < 2026033101) {
        upgrade_plugin_savepoint(true, 2026033101, 'local', 'rtocompliance');
    }

    // Version 3.8.44: UPGRADE FIX — Corrected upgrade.php savepoint for v3.8.40. The savepoint
    ...
    if ($oldversion < 2026033102) {
        upgrade_plugin_savepoint(true, 2026033102, 'local', 'rtocompliance');
    }

    // Version 3.8.45: Data Import tab added — moved WisenetImport/AVETMISS data import tool
    ...
    // Version 3.8.46: Data Import nav link — added 'Data Import' item to the plugin sidebar
    ...
    if ($oldversion < 2026040201) {
        upgrade_plugin_savepoint(true, 2026040201, 'local', 'rtocompliance');
    }

    // Version 3.8.47: TAS Section 2 AQF cohort selector + Industry Consultation dropdown helpers.
    ...
    if ($oldversion < 2026040202) {
        upgrade_plugin_savepoint(true, 2026040202, 'local', 'rtocompliance');
    }


    // Version 3.8.48: Student pre-enrolment suitability checklist system.
    ...
    if ($oldversion < 2026040203) {
        ...
        upgrade_plugin_savepoint(true, 2026040203, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040204) {
        // Version 3.8.49 — Bulk suitability checklist sending.
        ...
        upgrade_plugin_savepoint(true, 2026040204, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040205) {
        // Version 3.8.50 — AVETMISS Data Import: new data_import.php with PHP NAT file parser.
        ...
        upgrade_plugin_savepoint(true, 2026040205, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040206) {
        // Version 3.8.51: Suitability Checklist bug fixes — missing lang key, silent-reset guard,
        // graceful error pages, null-safe override view. No DB schema change.
        upgrade_plugin_savepoint(true, 2026040206, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040207) {
        // Version 3.8.52: Per-page deep-link help icons — render_nav_header() $help_anchor param,
        // support.php card id= anchors, all 40+ page calls updated. No DB schema change.
        upgrade_plugin_savepoint(true, 2026040207, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040208) {
        // Version 3.8.53: Quick Statistics cards at top of students, trainers, qualbuilder_results,
        // surveys, and complaints pages. stat-rose CSS added. No DB schema change.
        upgrade_plugin_savepoint(true, 2026040208, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040209) {
        // Version 3.8.54: Release checklist compliance pass — all stale ZIPs removed, BUILD_INFO.json
        // updated, upgrade.php savepoints back-filled for 109-112. No DB schema change.
        upgrade_plugin_savepoint(true, 2026040209, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040210) {
        // Version 3.8.55: SCHEMA SOURCE FIX — Four XMLDB CHAR NOT NULL columns in the AVETMISS
        ...
        upgrade_plugin_savepoint(true, 2026040210, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040211) {
        // Version 3.8.56: SETTINGS FIX — Removed duplicate admin_externalpage registration for
        ...
        upgrade_plugin_savepoint(true, 2026040211, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040212) {
        // Version 3.8.57: FEATURE — Collapsible left-hand sidebar navigation injected on all
        // plugin pages via the before_footer_html_generation hook. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040212, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040213) {
        // Version 3.8.58: FEATURE — Stats cards expanded to 2 full rows (8 cards each) on all
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040213, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040214) {
        // Version 3.8.59: FIX — Sidebar not appearing on plugin pages. The legacy
        ...
        upgrade_plugin_savepoint(true, 2026040214, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040215) {
        // Version 3.8.60: FIX — Sidebar theme-agnostic rewrite. Inline CSS is now injected
        ...
        upgrade_plugin_savepoint(true, 2026040215, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040216) {
        // Version 3.8.61: FIX — Sidebar not displaying on Moodle admin pages. Three-layer fix:
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040216, 'local', 'rtocompliance');
    }

    // Version 3.8.62: VERSION BUMP — Routine release increment. No code or DB schema changes.
    if ($oldversion < 2026040217) {
        upgrade_plugin_savepoint(true, 2026040217, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040218) {
        // Version 3.8.63 — SIDEBAR FIX: Replace position:fixed with DOM-restructuring flex layout.
        ...
        upgrade_plugin_savepoint(true, 2026040218, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040219) {
        // Version 3.8.64 — SIDEBAR FIX: The v6 DOM-restructuring approach in v3.8.63 was correct
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040219, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040220) {
        // Version 3.8.65 — ASQA 2025 DASHBOARD AUDIT: Full tester audit of dashboard (index.php)
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040220, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040300) {
        // Version 3.8.66 — SIDEBAR FIX (v7, definitive): Replaced DOM-restructuring flex approach
    ...
    if ($oldversion < 2026040301) {
        // Version 3.8.67 — SIDEBAR FIX (v8, server-side flex layout — definitive):
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040301, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040302) {
        // Version 3.8.68 — SIDEBAR FIX (v9): index.php (dashboard) and data_import.php were calling
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040302, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040303) {
        // Version 3.8.69 — FIX: complaints.php stats-card queries referenced a non-existent 'type'
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026040303, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040304) {
        // Version 3.8.70 — FIX: risk.php Risk Register table used html_writer::tag('br') with no
        ...
        upgrade_plugin_savepoint(true, 2026040304, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040305) {
        // Version 3.8.71 — AUDIT: No DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026040305, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040306) {
        // Version 3.8.72 — QPR COMPLIANCE FIX: No DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026040306, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040307) {
        // Version 3.8.73 — QPR VALIDATOR FIX: No DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026040307, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040308) {
        // Version 3.9.0 — v3.9.2: AI CONFIG AUDIT + 27 ASQA HELP BUTTONS. No DB schema changes.
        // v3.9.0:
        ...
        upgrade_plugin_savepoint(true, 2026040308, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026040400) {
        // Version 4.0.0 — WORLD-CLASS SIDEBAR + FULL AUDIT. No DB schema changes.
        //
        // SIDEBAR v9 (lib.php):
    ...
    // Version 4.0.1: SIDEBAR FIX — removed path guard from local_rtocompliance_render_sidebar()
    ...
    if ($oldversion < 2026040401) {
        upgrade_plugin_savepoint(true, 2026040401, 'local', 'rtocompliance');
    }

    // Version 4.0.2 - DIAGNOSTIC: render_sidebar() replaced with hard test stub to isolate
    // sidebar rendering issue. No DB schema changes.
    if ($oldversion < 2026040402) {
        upgrade_plugin_savepoint(true, 2026040402, 'local', 'rtocompliance');
    }

    // Version 4.0.3 - FIX: Removed empty($PAGE->url) guard from render_sidebar() — this was the
    ...
    if ($oldversion < 2026040403) {
        upgrade_plugin_savepoint(true, 2026040403, 'local', 'rtocompliance');
    }

    // Version 4.0.4 - FIX: Added global $CFG declaration and $currentpath assignment to
    ...
    if ($oldversion < 2026040404) {
        upgrade_plugin_savepoint(true, 2026040404, 'local', 'rtocompliance');
    }

    // Version 4.0.5 - UX: Sidebar auto-scrolls to active item on page load so the current
    // nav item is always visible regardless of sidebar scroll position. No DB changes.
    if ($oldversion < 2026040405) {
        upgrade_plugin_savepoint(true, 2026040405, 'local', 'rtocompliance');
    }

    // Version 4.0.6 - UX: Page auto-scrolls past Moodle header/nav chrome on load to show
    // plugin content immediately. Targets #region-main / .main-inner / [role=main].
    // No DB changes.
    if ($oldversion < 2026040406) {
        upgrade_plugin_savepoint(true, 2026040406, 'local', 'rtocompliance');
    }

    // Version 4.0.38: AMD SYNC FIX — amd/build/qualbuilder_edit.min.js was stale (MD5
    ...
    if ($oldversion < 2026040407) {
        upgrade_plugin_savepoint(true, 2026040407, 'local', 'rtocompliance');
    }

    // Version 4.0.40 — Three tester-reported fixes:
    ...
    // Version 4.0.41 — Testing engine column-name fixes (no user-visible changes):
    ...
    if ($oldversion < 2026041001) {
        upgrade_plugin_savepoint(true, 2026041001, 'local', 'rtocompliance');
    }

    // Version 4.0.42 — Release process correction: ZIP moved to public/downloads/ (correct server path for
    ...
    if ($oldversion < 2026041002) {
        upgrade_plugin_savepoint(true, 2026041002, 'local', 'rtocompliance');
    }

    // Version 4.0.43 — VERSION BUMP: No code changes. All 6 release locations updated and synced
    ...
    // Version 4.0.44 — AUTOFIX EXPANSION: Added three new auto-fix cases to testing.php:
    ...
    if ($oldversion < 2026041101) {
        upgrade_plugin_savepoint(true, 2026041101, 'local', 'rtocompliance');
    }

    // Version 4.0.45 — RTO COMPLIANCE TESTER UX FIXES:
    ...
    // Version 4.0.46 — MAINTENANCE VERSION BUMP: No code changes. All 6 release locations updated
    ...
    if ($oldversion < 2026041301) {
        upgrade_plugin_savepoint(true, 2026041301, 'local', 'rtocompliance');
    }

    // Version 4.0.47 — BUG FIXES: (1) Added missing lang strings volumeoflearning and
    ...
    if ($oldversion < 2026041302) {
        upgrade_plugin_savepoint(true, 2026041302, 'local', 'rtocompliance');
    }

    // Version 4.0.48 — BUG FIXES (Tester batch 1):
    ...
    if ($oldversion < 2026041303) {
        upgrade_plugin_savepoint(true, 2026041303, 'local', 'rtocompliance');
    }

    // Version 4.0.49 — Bug fixes (batch 2):
    ...
    if ($oldversion < 2026041304) {
        upgrade_plugin_savepoint(true, 2026041304, 'local', 'rtocompliance');
    }

    // Version 4.0.50 — ROOT-CAUSE CSS FIX: Replaced the entire broken mform accordion CSS block.
    ...
    if ($oldversion < 2026041305) {
        upgrade_plugin_savepoint(true, 2026041305, 'local', 'rtocompliance');
    }

    // Version 4.0.51 — CSS FULL-WIDTH FIX (CSS only, no DB changes):
    ...
    if ($oldversion < 2026041306) {
        upgrade_plugin_savepoint(true, 2026041306, 'local', 'rtocompliance');
    }

    // Version 4.0.53 — T&A REGISTER EXPANSION + 6 IMPROVEMENTS (PHP only, no DB schema changes):
    ...
    // Version 4.0.54 — TESTER FEEDBACK: 4-ISSUE FIX (PHP + DB schema changes):
    ...
    if ($oldversion < 2026041701) {
        ...
        upgrade_plugin_savepoint(true, 2026041701, 'local', 'rtocompliance');
    }

    // Version 4.0.55 — MARKETING_INFO STANDARD 2.1 RENAME + REMOVE COMPLAINTS BUTTON
    ...
    // Version 4.0.56 — STANDARDS 2.3-2.6 FUNCTIONAL SYSTEM + TRAINER INPUT AI AUTO-FILL +
    ...
    if ($oldversion < 2026042101) {
        upgrade_plugin_savepoint(true, 2026042101, 'local', 'rtocompliance');
    }

    // Version 4.0.52 — 28-ISSUE AUDIT FIX (PHP/CSS only, no DB schema changes):
    ...
    if ($oldversion < 2026042102) {
        upgrade_plugin_savepoint(true, 2026042102, 'local', 'rtocompliance');
    }

    // Version 4.0.53 - v4.0.56: No DB schema changes. See pluginConfig.ts changelog for details.
    if ($oldversion < 2026042103) {
        upgrade_plugin_savepoint(true, 2026042103, 'local', 'rtocompliance');
    }

    // Version 4.0.57: TESTER FEEDBACK FIXES (7 items, no DB schema changes).
    ...
    // Version 4.0.58: CRITICAL AMD ENCODING FIX -- Site admin primary/secondary navigation menus
    ...
    if ($oldversion < 2026042201) {
        upgrade_plugin_savepoint(true, 2026042201, 'local', 'rtocompliance');
    }

    // Version 4.0.59: SYSTEMIC CLICK FIX — admin_externalpage_setup() added to 32 admin sub-pages.
    ...
    if ($oldversion < 2026042202) {
        upgrade_plugin_savepoint(true, 2026042202, 'local', 'rtocompliance');
    }

    // Version 4.0.60 - THREE TESTER BUG FIXES: (1) TAS accordion click-blocking fixed via
    ...
    // Version 4.0.61 - VERSION BUMP: All 7 release locations synced. No DB schema changes.
    ...
    if ($oldversion < 2026042301) {
        upgrade_plugin_savepoint(true, 2026042301, 'local', 'rtocompliance');
    }

    // Version 4.0.62 - THREE BUG FIXES: (1) trainer_edit.php "Can't edit" crash fixed by
    ...
    // Version 4.0.63: FIX — settings.php missing isset($settings) guard around all 8 $ADMIN->fulltree
    ...
    // Version 4.0.64: MULTI-FIX — (1) Trainer table column reorder (Status under TGA moved after TAE
    ...
    if ($oldversion < 2026042501) {
        ...
        upgrade_plugin_savepoint(true, 2026042501, 'local', 'rtocompliance');
    }

    // Version 4.0.65: 8-BUG-FIX — (1) Student Records action buttons white-space:nowrap. (2) Site admin
    ...
    // Version 4.0.66: 8-BUG-FIX (actual code applied):
    ...
    if ($oldversion < 2026042601) {
        ...
        if (!$dbman->table_exists($table)) {
            // Signature: add_field($name, $type, $precision, $unsigned, $notnull, $sequence, $default)
            $table->add_field('id',            XMLDB_TYPE_INTEGER, '10',  null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            $table->add_field('userid',        XMLDB_TYPE_INTEGER, '10',  null, XMLDB_NOTNULL, null, '0');
            $table->add_field('token',         XMLDB_TYPE_CHAR,    '64',  null, XMLDB_NOTNULL);
        ...
        upgrade_plugin_savepoint(true, 2026042601, 'local', 'rtocompliance');
    }

    // Version 4.0.67: Version bump — no DB schema changes.
    ...
    if ($oldversion < 2026042602) {
        upgrade_plugin_savepoint(true, 2026042602, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026042603) {
        // Bootstrap 4/5 dual-attribute fix for dropdown menus in students.php and trainers.php.
        // No DB schema changes required.
        upgrade_plugin_savepoint(true, 2026042603, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042604) {
        ...
        upgrade_plugin_savepoint(true, 2026042604, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042605) {
        ...
        upgrade_plugin_savepoint(true, 2026042605, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042701) {
        ...
        upgrade_plugin_savepoint(true, 2026042701, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042702) {
        ...
        //   No DB schema changes.
        upgrade_plugin_savepoint(true, 2026042702, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042703) {
        ...
        //     No DB schema changes.
        upgrade_plugin_savepoint(true, 2026042703, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042704) {
        ...
        upgrade_plugin_savepoint(true, 2026042704, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026042705) {
        // Version bump — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026042705, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042706) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026042706, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026042707) {
        ...
        upgrade_plugin_savepoint(true, 2026042707, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042708) {
        ...
        //   No DB schema changes.
        upgrade_plugin_savepoint(true, 2026042708, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026042709) {
        // BUMP: Version increment only. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026042709, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026042710) {
        // FIX-MARKETING-POLICIES: Policies card redesign. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026042710, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042801) {
        ...
        upgrade_plugin_savepoint(true, 2026042801, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042802) {
        ...
        upgrade_plugin_savepoint(true, 2026042802, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042803) {
        ...
        upgrade_plugin_savepoint(true, 2026042803, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042901) {
        ...
        upgrade_plugin_savepoint(true, 2026042901, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042902) {
        ...
        upgrade_plugin_savepoint(true, 2026042902, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042903) {
        ...
        upgrade_plugin_savepoint(true, 2026042903, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042904) {
        ...
        upgrade_plugin_savepoint(true, 2026042904, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042905) {
        ...
        upgrade_plugin_savepoint(true, 2026042905, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042906) {
        ...
        upgrade_plugin_savepoint(true, 2026042906, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042907) {
        ...
        upgrade_plugin_savepoint(true, 2026042907, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042908) {
        ...
        //
        // No DB schema changes in this upgrade step.
        upgrade_plugin_savepoint(true, 2026042908, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026042909) {
        ...
        upgrade_plugin_savepoint(true, 2026042909, 'local', 'rtocompliance');
    }

    // Version 4.2.12 (2026043000001): Remediation upgrade step — ensures local_rtocompliance_ai_survey
    ...
    if ($oldversion < 2026043001) {
        ...
        upgrade_plugin_savepoint(true, 2026043001, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043002) {
        // Version 4.2.14 remediation (30 Apr 2026):
        ...
        upgrade_plugin_savepoint(true, 2026043002, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043003) {
        // Version 4.2.15 follow-up remediation (30 Apr 2026):
        ...
        upgrade_plugin_savepoint(true, 2026043003, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043004) {
        // Version 4.2.16 namespace fix (30 Apr 2026):
        ...
        upgrade_plugin_savepoint(true, 2026043004, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043005) {
        // Version 4.2.17 Survey AI HTTP-Array fix (30 Apr 2026):
        ...
        //   No DB schema change.
        upgrade_plugin_savepoint(true, 2026043005, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043006) {
        // Version 4.2.18 USI platform error surfacing + TAS dropdown layout fix (30 Apr 2026):
        ...
        //   No DB schema change.
        upgrade_plugin_savepoint(true, 2026043006, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043007) {
        // Version 4.2.19 Validation methods/risk-factors persistence fix (30 Apr 2026):
        ...
        //   first edit.
        upgrade_plugin_savepoint(true, 2026043007, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043008) {
        // Version 4.2.20 Survey AI error message + Run AI Analysis button colour fixes
        // (30 Apr 2026):
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026043008, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043009) {
        // Version 4.2.21 Student Records save + TAS overlap fix (30 Apr 2026):
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026043009, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043010) {
        // Version 4.2.22 Dashboard "Create Your First Enrolment" tile fix (30 Apr 2026):
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026043010, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043011) {
        // Version 4.2.23 Industry Consultation quick-add helper overflow fix (30 Apr 2026):
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026043011, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043012) {
        // Version 4.2.24 AI Analysis silent-no-op + upgrade savepoint order fixes (30 Apr 2026):
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026043012, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043013) {
        // Version 4.2.26 BUG-SR-OUTCOME-SAVE-3 silent enrolment-save revert fix (30 Apr 2026):
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026043013, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043014) {
        // Version 4.2.27 BUG-SR-OUTCOME-AUTOREVERT silent enrolment-overwrite fix (30 Apr 2026):
        ...
        $field = new xmldb_field(
            'manualoutcome', XMLDB_TYPE_INTEGER, '1', null,
            XMLDB_NOTNULL, null, '0', 'outcomeidentifier');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2026043014, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043015) {
        // Version 4.2.29 BUG-RESULTS-NOID-BOUNCE + BUG-SURVEY-AI-NOSHOW (30 Apr 2026):
        ...
        upgrade_plugin_savepoint(true, 2026043015, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043016) {
        // Version 4.2.30 PER-TENANT-USI + ROLE-SPLIT + NAV-PRIMARY (30 Apr 2026):
        ...
        upgrade_plugin_savepoint(true, 2026043016, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026043017) {
        // Version 4.2.31 USI-DISCOVERABILITY (30 Apr 2026):
        ...
        //   installed_version.
        upgrade_plugin_savepoint(true, 2026043017, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050100) {
        // Version 4.2.32 BUG-SR-OUTCOME-SAVE-4 + BUG-SURVEY-AI-NORESP (1 May 2026):
    ...
    if ($oldversion < 2026050101) {
        // Version 4.2.33 BUG-SR-OUTCOME-SAVE-5 + BUG-SURVEY-AI-NORESP-2 (1 May 2026, AM):
        //   Two pure-PHP UX fixes, no schema change:
        ...
        upgrade_plugin_savepoint(true, 2026050101, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050102) {
        // Version 4.2.34 BUG-SURVEY-AI-2CLICK (1 May 2026, AM):
        ...
        //   Savepoint only exists to bump installed_version.
        upgrade_plugin_savepoint(true, 2026050102, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050103) {
        // Version 4.2.35 BUG-SURVEY-AI-NOSHOW-2 (1 May 2026, mid-AM):
        ...
        //   Savepoint only exists to bump installed_version.
        upgrade_plugin_savepoint(true, 2026050103, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050104) {
        // Version 4.2.36 CERTIFICATES-REDESIGN (1 May 2026):
        ...
        upgrade_plugin_savepoint(true, 2026050104, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050105) {
        ...
        upgrade_plugin_savepoint(true, 2026050105, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050106) {
        ...
        upgrade_plugin_savepoint(true, 2026050106, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050107) {
        ...
        // No DB changes.
        upgrade_plugin_savepoint(true, 2026050107, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050108) {
        ...
        upgrade_plugin_savepoint(true, 2026050108, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050109) {
        upgrade_plugin_savepoint(true, 2026050109, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050201) {
        upgrade_plugin_savepoint(true, 2026050201, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050202) {
        upgrade_plugin_savepoint(true, 2026050202, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050203) {
        upgrade_plugin_savepoint(true, 2026050203, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050204) {
        upgrade_plugin_savepoint(true, 2026050204, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050205) {
        ...
        upgrade_plugin_savepoint(true, 2026050205, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050206) {
        // Version 4.2.48 BUG-MAY2-AUDIT — code-only release. No schema change.
        ...
        upgrade_plugin_savepoint(true, 2026050206, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050207) {
        // Version 4.2.49 BUG-MAY2-AUDIT2 — code-only release. No schema change.
        ...
        upgrade_plugin_savepoint(true, 2026050207, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050208) {
        // Version 4.2.50 — Suitability 4-stage rebuild + pluggable LLN adapter.
        ...
        upgrade_plugin_savepoint(true, 2026050208, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050209) {
        // Version 4.2.51 DECLARATION-SELFAUDIT - persist the typed-name signature
        ...
        upgrade_plugin_savepoint(true, 2026050209, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050210) {
        // Version 4.2.52 DECLARATION-SELFAUDIT-B7-SPACE - no schema change, just a
        ...
        upgrade_plugin_savepoint(true, 2026050210, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050211) {
        // Version 4.2.53 FIX-MAY2-IDX - savepoint marker only.  The actual fix
        ...
        upgrade_plugin_savepoint(true, 2026050211, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050212) {
        ...
        upgrade_plugin_savepoint(true, 2026050212, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050213) {
        ...
        // change.
        upgrade_plugin_savepoint(true, 2026050213, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050214) {
        ...
        upgrade_plugin_savepoint(true, 2026050214, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050215) {
        ...
        upgrade_plugin_savepoint(true, 2026050215, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050216) {
        ...
        try {
            ...
            debugging(
                'v4.2.58 default template seed failed: ' . $e->getMessage(),
                DEBUG_DEVELOPER);
        }
        upgrade_plugin_savepoint(true, 2026050216, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050217) {
        ...
        try {
            ...
            debugging(
                'v4.2.59 default template refresh failed: ' . $e->getMessage(),
                DEBUG_DEVELOPER);
        }
        upgrade_plugin_savepoint(true, 2026050217, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050218) {
        ...
        try {
            ...
            debugging(
                'v4.2.60 testamur portrait re-seed failed: ' . $e->getMessage(),
                DEBUG_DEVELOPER);
        }
        upgrade_plugin_savepoint(true, 2026050218, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050219) {
        ...
        try {
            ...
            debugging(
                'v4.2.61 ASQA starter re-seed failed: ' . $e->getMessage(),
                DEBUG_DEVELOPER);
        }
        upgrade_plugin_savepoint(true, 2026050219, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050220) {
        ...
        try {
            ...
            debugging(
                'v4.2.62 ASQA starter recovery re-seed failed: ' . $e->getMessage(),
                DEBUG_DEVELOPER);
        }
        upgrade_plugin_savepoint(true, 2026050220, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050221) {
        ...
        $tmplidfield = new xmldb_field(
            'certtmplid', XMLDB_TYPE_INTEGER, '10',
        ...
        upgrade_plugin_savepoint(true, 2026050221, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050222) {
        ...
        upgrade_plugin_savepoint(true, 2026050222, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050223) {
        ...
        upgrade_plugin_savepoint(true, 2026050223, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050224) {
        ...
        upgrade_plugin_savepoint(true, 2026050224, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050225) {
        ...
        upgrade_plugin_savepoint(true, 2026050225, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050226) {
        ...
        upgrade_plugin_savepoint(true, 2026050226, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050227) {
        ...
        upgrade_plugin_savepoint(true, 2026050227, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050228) {
        ...
        upgrade_plugin_savepoint(true, 2026050228, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050229) {
        ...
        upgrade_plugin_savepoint(true, 2026050229, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050230) {
        ...
        upgrade_plugin_savepoint(true, 2026050230, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050231) {
        ...
        upgrade_plugin_savepoint(true, 2026050231, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050232) {
        ...
        upgrade_plugin_savepoint(true, 2026050232, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050233) {
        ...
        upgrade_plugin_savepoint(true, 2026050233, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050234) {
        ...
        // No schema change. No AMD change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050234, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050235) {
        ...
        // No schema change. No AMD change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050235, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050236) {
        ...
        // No schema change. No AMD change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050236, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050237) {
        ...
        upgrade_plugin_savepoint(true, 2026050237, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050238) {
        ...
        upgrade_plugin_savepoint(true, 2026050238, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050239) {
        ...
        upgrade_plugin_savepoint(true, 2026050239, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050240) {
        ...
        // Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050240, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050241) {
        ...
        // No schema change. No AMD change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050241, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050301) {
        ...
        // change. No AMD change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050301, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050302) {
        ...
        // change. No AMD change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050302, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050303) {
        ...
        upgrade_plugin_savepoint(true, 2026050303, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050304) {
        ...
        upgrade_plugin_savepoint(true, 2026050304, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050305) {
        ...
        upgrade_plugin_savepoint(true, 2026050305, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050306) {
        ...
        upgrade_plugin_savepoint(true, 2026050306, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050307) {
        ...
        // SCHEMA: NO change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050307, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050308) {
        ...
        // SCHEMA: NO change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050308, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050309) {
        ...
        // SCHEMA: NO change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050309, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050310) {
        // CHECKPOINT-LOGGING (v4.4.31): savepoint only — superseded by v4.4.32
        upgrade_plugin_savepoint(true, 2026050310, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050311) {
        ...
        upgrade_plugin_savepoint(true, 2026050311, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050312) {
        ...
        // SCHEMA: NO change. SETTINGS: NO change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050312, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050313) {
        ...
        // SCHEMA: NO change. SETTINGS: NO change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050313, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050314) {
        ...
        // SCHEMA: NO change. SETTINGS: NO change. Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050314, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050315) {
        ...
        upgrade_plugin_savepoint(true, 2026050315, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050316) {
        ...
        // Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050316, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050317) {
        ...
        // SCHEMA: NO change. SETTINGS: NO change. AMD: NO change.
        // Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050317, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050318) {
        ...
        upgrade_plugin_savepoint(true, 2026050318, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050319) {
        ...
        upgrade_plugin_savepoint(true, 2026050319, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050320) {
        ...
        // Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050320, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050321) {
        ...
        // Savepoint is a marker only.
        upgrade_plugin_savepoint(true, 2026050321, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050322) {
        ...
        upgrade_plugin_savepoint(true, 2026050322, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050501) {
        ...
        upgrade_plugin_savepoint(true, 2026050501, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050502) {
        ...
        upgrade_plugin_savepoint(true, 2026050502, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050503) {
        ...
        upgrade_plugin_savepoint(true, 2026050503, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050504) {
        ...
        upgrade_plugin_savepoint(true, 2026050504, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050505) {
        ...
        upgrade_plugin_savepoint(true, 2026050505, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050506) {
        ...
        upgrade_plugin_savepoint(true, 2026050506, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050507) {
        ...
        upgrade_plugin_savepoint(true, 2026050507, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050508) {
        ...
        upgrade_plugin_savepoint(true, 2026050508, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050509) {
        ...
        upgrade_plugin_savepoint(true, 2026050509, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050510) {
        ...
        upgrade_plugin_savepoint(true, 2026050510, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050511) {
        ...
        upgrade_plugin_savepoint(true, 2026050511, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050512) {
        ...
        upgrade_plugin_savepoint(true, 2026050512, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050513) {
        ...
        upgrade_plugin_savepoint(true, 2026050513, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050514) {
        ...
        upgrade_plugin_savepoint(true, 2026050514, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050701) {
        ...
        upgrade_plugin_savepoint(true, 2026050701, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050702) {
        ...
        // SCHEMA: NO change. AMD: NO change. Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026050702, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050703) {
        ...
        upgrade_plugin_savepoint(true, 2026050703, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050704) {
        ...
        upgrade_plugin_savepoint(true, 2026050704, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050705) {
        ...
        upgrade_plugin_savepoint(true, 2026050705, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050706) {
        ...
        // SCHEMA: NO change. AMD: NO change. Marker savepoint.
        upgrade_plugin_savepoint(true, 2026050706, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050707) {
        ...
        // SCHEMA: NO change. AMD: NO change. Marker savepoint.
        upgrade_plugin_savepoint(true, 2026050707, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050805) {
        ...
        // SCHEMA: NO change. AMD: NO change. Marker savepoint.
        upgrade_plugin_savepoint(true, 2026050805, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050806) {
        ...
        // SCHEMA: NO change. AMD: NO change. Marker savepoint.
        upgrade_plugin_savepoint(true, 2026050806, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050807) {
        ...
        upgrade_plugin_savepoint(true, 2026050807, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050808) {
        ...
        upgrade_plugin_savepoint(true, 2026050808, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050809) {
        ...
        // SCHEMA: NO change. AMD: NO change. Marker savepoint.
        upgrade_plugin_savepoint(true, 2026050809, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026050810) {
        ...
        upgrade_plugin_savepoint(true, 2026050810, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026050811) {
        ...
        upgrade_plugin_savepoint(true, 2026050811, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051001) {
        ...
        // SCHEMA: NO change. AMD: NO change.
        upgrade_plugin_savepoint(true, 2026051001, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051002) {
        // Version 4.5.98: Two bugs from errors_10_May_2026_(2).docx
        ...
        // SCHEMA: NO change. AMD: NO change.
        upgrade_plugin_savepoint(true, 2026051002, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051200) {
        // Version 4.5.100: FIX-QB-GROUP-RULES-SERVERSIDE (12 May 2026)
    ...
    if ($oldversion < 2026051201) {
        // Version 4.6.101: MULTI-UNIT-SOA (12 May 2026)
        ...
        upgrade_plugin_savepoint(true, 2026051201, 'local', 'rtocompliance');
    }

    // Version 4.6.102: FIX-CURL-BATCH — usi_platform_client.php, packagingrules_validator.php, and
    ...
    if ($oldversion < 2026051202) {
        upgrade_plugin_savepoint(true, 2026051202, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051400) {
        // Version 4.9.108 STUDENT-DOC-REPOSITORY (14 May 2026)
    ...
    if ($oldversion < 2026051401) {
        // Version 4.9.109 FIX-SMART-DETECT-BANNER — PHP/CSS only. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051401, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051402) {
        // Version 4.9.110 FIX-PURCHASE-CREDITS-URL — No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051402, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051403) {
        // Version 4.9.111 FIX-NAT00080-AVETMISS8 — No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051403, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051404) {
        // Version 4.9.112 FIX-NAT00080-EXTENDED — No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051404, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051405) {
        // Version 4.9.113 FIX-NAT00080-WISENET — No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051405, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051406) {
        // Version 4.9.114 NAT00080-CONFIRM-STEP — No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051406, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051407) {
        // Version 4.9.115 AVETMISS-PARCHMENT-VERIFY — No DB schema changes.
        // verify.php now cross-references avetmiss_completion parchment numbers
        // (AVETMISS DE 515) via USI join for staff viewers.
        upgrade_plugin_savepoint(true, 2026051407, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051408) {
        // Version 4.9.116 FIX-NAT00080-GENDER-AT — No DB schema changes.
        // '@' (AVETMISS "not stated" gender code) now accepted in both sex parse
        // paths; no longer triggers sex_not_stated data-issue flag.
        upgrade_plugin_savepoint(true, 2026051408, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051409) {
        // Version 4.9.117 AUTOENROL-WIZARD — No DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026051409, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051410) {
        // Version 4.9.118 AUTOENROL-HARDENING — No DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026051410, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051411) {
        // Version 4.9.119 FIX-LINK-COURSES-UX — No DB schema changes.
        ...
        upgrade_plugin_savepoint(true, 2026051411, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051500) {
        // Version 4.9.124 FIX-AUTOENROL-WIZARD-UX — No DB schema changes.
    ...
    if ($oldversion < 2026051501) {
        // Version 4.9.125 — NAT-IMPORT-PARSE-HARDENING + DIAG-PAGE (16 May 2026)
        ...
        upgrade_plugin_savepoint(true, 2026051501, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051601) {
        ...
        upgrade_plugin_savepoint(true, 2026051601, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051602) {
        ...
        upgrade_plugin_savepoint(true, 2026051602, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051603) {
        ...
        upgrade_plugin_savepoint(true, 2026051603, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051604) {
        ...
        upgrade_plugin_savepoint(true, 2026051604, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051605) {
        ...
        upgrade_plugin_savepoint(true, 2026051605, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051606) {
        ...
        upgrade_plugin_savepoint(true, 2026051606, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051607) {
        ...
        upgrade_plugin_savepoint(true, 2026051607, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051608) {
        ...
        upgrade_plugin_savepoint(true, 2026051608, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051609) {
        ...
        upgrade_plugin_savepoint(true, 2026051609, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051610) {
        ...
        upgrade_plugin_savepoint(true, 2026051610, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051611) {
        ...
        upgrade_plugin_savepoint(true, 2026051611, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051612) {
        ...
        foreach ($rs as $stud) {
            ...
            if (!empty($issues)) {
                $DB->update_record(
                    'local_rtocompliance_avetmiss_student', (object)[
                        'id'              => $stud->id,
                        'hasdataissues'   => 1,
        ...
        upgrade_plugin_savepoint(true, 2026051612, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051613) {
        ...
        upgrade_plugin_savepoint(true, 2026051613, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051614) {
        ...
        upgrade_plugin_savepoint(true, 2026051614, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051615) {
        ...
        upgrade_plugin_savepoint(true, 2026051615, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051616) {
        ...
        upgrade_plugin_savepoint(true, 2026051616, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051701) {
        ...
        upgrade_plugin_savepoint(true, 2026051701, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051702) {
        ...
        upgrade_plugin_savepoint(true, 2026051702, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051703) {
        ...
        upgrade_plugin_savepoint(true, 2026051703, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051704) {
        ...
        upgrade_plugin_savepoint(true, 2026051704, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051705) {
        ...
        upgrade_plugin_savepoint(true, 2026051705, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051706) {
        ...
        upgrade_plugin_savepoint(true, 2026051706, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051707) {
        ...
        upgrade_plugin_savepoint(true, 2026051707, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051901) {
        ...
        upgrade_plugin_savepoint(true, 2026051901, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051902) {
        ...
        upgrade_plugin_savepoint(true, 2026051902, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026051903) {
        ...
        upgrade_plugin_savepoint(true, 2026051903, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051904) {
        ...
        upgrade_plugin_savepoint(true, 2026051904, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051905) {
        ...
        // No schema change.
        upgrade_plugin_savepoint(true, 2026051905, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026051906) {
        ...
        upgrade_plugin_savepoint(true, 2026051906, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052001) {
        ...
        upgrade_plugin_savepoint(true, 2026052001, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052002) {
        ...
        upgrade_plugin_savepoint(true, 2026052002, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052003) {
        ...
        // No DB schema changes. Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052003, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052004) {
        ...
        upgrade_plugin_savepoint(true, 2026052004, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052005) {
        // FIX-AUTOENROL-ENROLFAILED-VISIBLE + FIX-AUTOENROL-PLACEHOLDER-COLLISION (v4.9.173):
        // no DB schema changes. Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052005, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052006) {
        // FIX-AUTOENROL-MATCH-FALLBACK (v4.9.172): no DB schema changes.
        // Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052006, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052007) {
        ...
        upgrade_plugin_savepoint(true, 2026052007, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052008) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052008, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052009) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052009, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052010) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052010, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052011) {
        ...
        local_rtocompliance_upgrade_widen_usi($dbman);
        upgrade_plugin_savepoint(true, 2026052011, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052012) {
        // CLEANUP (v4.9.151): removed diagnostic scaffolding from data_import.php. No schema change.
        upgrade_plugin_savepoint(true, 2026052012, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052013) {
        ...
        upgrade_plugin_savepoint(true, 2026052013, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052014) {
        ...
        upgrade_plugin_savepoint(true, 2026052014, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052101) {
        ...
        upgrade_plugin_savepoint(true, 2026052101, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052102) {
        ...
        upgrade_plugin_savepoint(true, 2026052102, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052103) {
        ...
        upgrade_plugin_savepoint(true, 2026052103, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052104) {
        ...
        upgrade_plugin_savepoint(true, 2026052104, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052105) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026052105, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052106) {
        ...
        upgrade_plugin_savepoint(true, 2026052106, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052107) {
        ...
        upgrade_plugin_savepoint(true, 2026052107, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052108) {
        ...
        upgrade_plugin_savepoint(true, 2026052108, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052109) {
        ...
        upgrade_plugin_savepoint(true, 2026052109, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052110) {
        ...
        upgrade_plugin_savepoint(true, 2026052110, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052111) {
        ...
        upgrade_plugin_savepoint(true, 2026052111, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052112) {
        ...
        upgrade_plugin_savepoint(true, 2026052112, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052113) {
        ...
        upgrade_plugin_savepoint(true, 2026052113, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052114) {
        ...
        // PHP + CSS only. No AMD or DB changes.
        upgrade_plugin_savepoint(true, 2026052114, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052115) {
        ...
        upgrade_plugin_savepoint(true, 2026052115, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052116) {
        ...
        // CSS-only. No DB changes.
        upgrade_plugin_savepoint(true, 2026052116, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052301) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026052301, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052302) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026052302, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052303) {
        ...
        upgrade_plugin_savepoint(true, 2026052303, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052304) {
        ...
        //     No DB schema changes.
        upgrade_plugin_savepoint(true, 2026052304, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052305) {
        ...
        upgrade_plugin_savepoint(true, 2026052305, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052306) {
        ...
        upgrade_plugin_savepoint(true, 2026052306, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052307) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052307, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052308) {
        ...
        upgrade_plugin_savepoint(true, 2026052308, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052309) {
        ...
        upgrade_plugin_savepoint(true, 2026052309, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052310) {
        // DIAG-XHEADER (v4.9.149): X-RTOC-Step response headers added to data_import.php
        // at 6 checkpoints. No schema change.
        upgrade_plugin_savepoint(true, 2026052310, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026052311) {
        ...
        upgrade_plugin_savepoint(true, 2026052311, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052701) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052701, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052801) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026052801, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026052802) {
        ...
        upgrade_plugin_savepoint(true, 2026052802, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026053001) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026053001, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026053002) {
        ...
        upgrade_plugin_savepoint(true, 2026053002, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026053003) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026053003, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053004) {
        // Version 5.2.71–v5.2.81: Various fixes (sidebar icons, missing nav headers on admin pages,
        // SVG icon sizing, USI verification, cert builder, content/quiz plugin updates).
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053004, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053005) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053005, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053006) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053006, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053007) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053007, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053008) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053008, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053009) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053009, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053010) {
        ...
        // No DB schema changes — marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053010, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053011) {
        ...
        upgrade_plugin_savepoint(true, 2026053011, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053012) {
        ...
        // No DB schema changes. Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053012, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053013) {
        ...
        upgrade_plugin_savepoint(true, 2026053013, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053014) {
        ...
        // No DB schema changes. Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053014, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026053015) {
        ...
        // No DB schema changes. Marker savepoint only.
        upgrade_plugin_savepoint(true, 2026053015, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026060101) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060101, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026060102) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060102, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026060103) {
        ...
        upgrade_plugin_savepoint(true, 2026060103, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026060104) {
        ...
        upgrade_plugin_savepoint(true, 2026060104, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026060201) {
        ...
        upgrade_plugin_savepoint(true, 2026060201, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026060202) {
        ...
        upgrade_plugin_savepoint(true, 2026060202, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060203) {
        // ARCHIVE-INDEX-PHASE3 (v5.9.0): No DB schema changes — code-only release.
        // Marker savepoint so Moodle advances the installed version correctly.
        upgrade_plugin_savepoint(true, 2026060203, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060204) {
        // Version 5.9.1: Fix missing upgrade savepoints (2026060200010/011 were absent,
        ...
        upgrade_plugin_savepoint(true, 2026060204, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060205) {
        // Version 5.9.2: Two critical data_import.php bugs fixed.
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060205, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060206) {
        // Version 5.9.3: Version bump to provide a clean upgrade target for sites stuck in
        // the HTTP 500 upgrade loop caused by v5.8.0/v5.9.0 missing savepoints.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060206, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060207) {
        // Version 5.9.4: DIAG build — PHP error display added to data_import.php.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060207, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060208) {
        // Version 5.9.5: Adds diag_500.php standalone diagnostic.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060208, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060209) {
        // Version 5.9.6: Improves diag_500.php with session dump + log file + session lock release.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060209, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060210) {
        // Version 5.9.7: Adds breadcrumb error_log() to data_import.php; diag_500.php reads log files.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060210, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060211) {
        // Version 5.9.8: FIX-DIAG-HEADERS-SENT: diag_500.php ob_start() before any output.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060211, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060212) {
        // Version 5.9.9: FIX-DIAG-DB-QUERY: removed invalid $DB->record_exists('external_pages') call.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060212, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060213) {
        // Version 5.9.10: FIX-DIAG-SHUTDOWN-HANDLER: shutdown fired on success, stealing output.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060213, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060214) {
        // Version 5.9.11: FIX-DIAG-DB-CHECKPOINT: DB-based breadcrumb logging in data_import.php.
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060214, 'local', 'rtocompliance');
    }
    ...
    // Version 5.9.13: OPCACHE-FIX — adds opcache_fix.php. No DB schema change.
    ...
    // Version 5.9.15: UX — expanded "Archive index is empty" warning with steps + button. No DB schema change.
    if ($oldversion < 2026060302) {
        upgrade_plugin_savepoint(true, 2026060302, 'local', 'rtocompliance');
    }

    // Version 5.9.16: Removed diagnostic/testing PHP files (opcache_fix, diag_*, test_data, testing, trainers_diag). No DB schema change.
    if ($oldversion < 2026060303) {
        upgrade_plugin_savepoint(true, 2026060303, 'local', 'rtocompliance');
    }

    // Version 5.9.17: AUTO-UNHIDE hidden archive categories at enrolment time. Preview now shows AUTO (not REVIEW) when category is hidden but otherwise ready.
    if ($oldversion < 2026060304) {
        upgrade_plugin_savepoint(true, 2026060304, 'local', 'rtocompliance');
    }

    // Version 5.9.18: Added SC001 and ABC12345 to qual_to_family map.
    if ($oldversion < 2026060305) {
        upgrade_plugin_savepoint(true, 2026060305, 'local', 'rtocompliance');
    }

    // Version 5.9.19: Fixed $m[2] → $m[1] bug in archive_detect_year_sem(); 2-digit years (e.g. "22 XYZ S1")
    // were always resolving to year=2020 instead of 2022, causing false "duplicate" groupings.
    if ($oldversion < 2026060306) {
        upgrade_plugin_savepoint(true, 2026060306, 'local', 'rtocompliance');
    }

    // Version 5.9.20: Archive index now skips categories with no S1/S2 semester (CPD/CBC/Summer School noise).
    ...
    if ($oldversion < 2026060307) {
        upgrade_plugin_savepoint(true, 2026060307, 'local', 'rtocompliance');
    }

    // Version 5.9.21: FIX — conflict groups were all keyed by 'family' in get_records_sql(), causing all but
    ...
    if ($oldversion < 2026060308) {
        upgrade_plugin_savepoint(true, 2026060308, 'local', 'rtocompliance');
    }

    // Version 5.9.22: After NAT enrolment completes, a "Hide Archive Courses Now" card appears on the
    ...
    if ($oldversion < 2026060309) {
        upgrade_plugin_savepoint(true, 2026060309, 'local', 'rtocompliance');
    }

    // Version 5.9.23: Qual-code-aware archive matching.  When multiple archive categories exist for the
    ...
    if ($oldversion < 2026060310) {
        upgrade_plugin_savepoint(true, 2026060310, 'local', 'rtocompliance');
    }

    // Version 5.9.24: Group-splitting for mixed-qual NAT groups.  Previously, when a NAT file contained
    ...
    if ($oldversion < 2026060311) {
        upgrade_plugin_savepoint(true, 2026060311, 'local', 'rtocompliance');
    }

    // Version 5.9.25: Archive Index now correctly counts and displays "truly unresolved" conflicts —
    ...
    if ($oldversion < 2026060312) {
        upgrade_plugin_savepoint(true, 2026060312, 'local', 'rtocompliance');
    }

    // Version 5.9.26: Auto-routeability check now uses breadcrumb-based qual-code detection.
    ...
    if ($oldversion < 2026060313) {
        upgrade_plugin_savepoint(true, 2026060313, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060314) {
        // Version 5.9.27: Verify NAT Data tab — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026060314, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060315) {
        // Version 5.9.28: Fix Verify NAT Data DB queries — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026060315, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060316) {
        // Version 5.9.29: Fix name blank for fixed-width NAT files — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026060316, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060317) {
        // Version 5.9.30: Verify NAT Data — add NAT00085 email-based Moodle account matching — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026060317, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060318) {
        // Version 5.9.31: Backfill Student Records action — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026060318, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060319) {
        // Version 5.9.32: In-context help banners on NAT Import, Confirm, Auto-Enrol (Step 3), and Verify NAT Data pages — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026060319, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060320) {
        // Version 5.9.33: Qual Builder prerequisite check on NAT Import page — shows blocking warning if no quals set up, or a subtle reminder with count if set up. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060320, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060321) {
        // Version 5.9.34: USI Verification setup popup on Student Records page — auto-shows modal when API not connected; softer banner when API connected but cert not uploaded. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060321, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060322) {
        // Version 5.9.35: FIX — "Open API Connection settings" button used wrong section name (local_rtocompliance vs local_rtocompliance_api), causing a 404. Fixed in usi_settings.php and tas_edit.php. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060322, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060323) {
        // Version 5.9.36: IMPROVE — Platform Webhook Key field in API Settings now clearly marked as optional with instructions. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060323, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060324) {
        // Version 5.9.37: FIX — Removed "SaaS dashboard" reference from USI settings tip; RTOs do not have access to lms-labs.com/admin. Tip now correctly explains they contact their account manager. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060324, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060325) {
        // Version 5.9.38: CSS — consistent padding on all edges of every plugin page. .rtoc-main-content now has padding: 0 24px 40px 24px (was left-only). Nav header margins updated to -1.5rem horizontal. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060325, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060326) {
        // Version 5.9.39: COPY — Simplified webhook key descriptions to plain English. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026060326, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060327) {
        // Version 5.9.40: UX — Removed all disabled/greyed-out gates from USI machine credential form. No DB changes.
        upgrade_plugin_savepoint(true, 2026060327, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060328) {
        // Version 5.9.41: UX — Added permanent Backfill Student Records shortcut button on main Data Import page. No DB changes.
        upgrade_plugin_savepoint(true, 2026060328, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060329) {
        // Version 5.9.42: DOCS/UX — Full workflow guide in support.php and expanded 4-column how-it-works card in data_import.php. No DB changes.
        upgrade_plugin_savepoint(true, 2026060329, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060330) {
        // Version 5.9.43: STATE FUNDING — Add state-specific fields for Australian state/territory
        ...
        upgrade_plugin_savepoint(true, 2026060330, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060400) {
        // Version 5.9.44: No DB schema changes. settings.php namespace fix for avetmiss_codes.
        upgrade_plugin_savepoint(true, 2026060400, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060401) {
        // Version 5.9.45: No DB schema changes. State-funding audit fixes (3 bugs).
        upgrade_plugin_savepoint(true, 2026060401, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060402) {
        // Version 5.9.46: No DB schema changes. settings.php explicit require_once for avetmiss_codes.
        upgrade_plugin_savepoint(true, 2026060402, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060403) {
        // Version 5.9.47: No DB schema changes. State Funding UX overhaul + private-name purge.
        upgrade_plugin_savepoint(true, 2026060403, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060404) {
        // Version 5.9.48: No DB schema changes. Expanded state/territory regulator dropdown from 4 to 11 entries.
        upgrade_plugin_savepoint(true, 2026060404, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060405) {
        // Version 5.9.49: No DB schema changes. Fixed State Funding tab in plugin_settings.php (missing from allowlist + tabs).
        upgrade_plugin_savepoint(true, 2026060405, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060406) {
        // Version 5.9.50: No DB schema changes. support.php HTTP 500 fix: added diagnostic breadcrumb logger
        // + function_exists guard on support_icon() + missing emerald CSS classes.
        upgrade_plugin_savepoint(true, 2026060406, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060407) {
        // Version 5.9.51: No DB schema changes. ROOT-CAUSE FIX — settings.php require_once avetmiss_codes.php
        ...
        upgrade_plugin_savepoint(true, 2026060407, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060408) {
        // Version 5.9.52: No DB schema changes. ENROL-CONTINUING-ONLY — auto-enrol doenrol action now only
        ...
        upgrade_plugin_savepoint(true, 2026060408, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060900) {
        // Version 5.9.54: No DB schema changes. CSS-FIX — Fixed nav-header negative margins.
        upgrade_plugin_savepoint(true, 2026060900, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060901) {
        // Version 5.9.54: No DB schema changes. (duplicate savepoint — padding belt-and-suspenders).
        upgrade_plugin_savepoint(true, 2026060901, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060902) {
        // Version 5.9.55: No DB schema changes. ROOT-CAUSE FIX — lib.php double-load on symlinked Moodle.
        // Added LOCAL_RTOCOMPLIANCE_LIB_LOADED define guard at top of lib.php.
        upgrade_plugin_savepoint(true, 2026060902, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060903) {
        // Version 5.9.56: No DB schema changes. BREADCRUMB-LAYOUT-FIX — fixed nav-header items stacking
        ...
        upgrade_plugin_savepoint(true, 2026060903, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026060904) {
        // Version 5.9.57: No DB schema changes. ENROL-UNIT-ACCURATE — new "Unit-accurate enrolment"
        ...
        upgrade_plugin_savepoint(true, 2026060904, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026061000) {
        // Version 5.9.58: No DB schema changes. FIX-SETTINGS-FUNCGUARD — replaced the bare
    ...
    if ($oldversion < 2026062501) {
        ...
        upgrade_plugin_savepoint(true, 2026062501, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062502) {
        ...
        upgrade_plugin_savepoint(true, 2026062502, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062503) {
        ...
        upgrade_plugin_savepoint(true, 2026062503, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062504) {
        ...
        upgrade_plugin_savepoint(true, 2026062504, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062505) {
        ...
        upgrade_plugin_savepoint(true, 2026062505, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062506) {
        ...
        upgrade_plugin_savepoint(true, 2026062506, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062507) {
        ...
        upgrade_plugin_savepoint(true, 2026062507, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062508) {
        ...
        upgrade_plugin_savepoint(true, 2026062508, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062509) {
        ...
        upgrade_plugin_savepoint(true, 2026062509, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062510) {
        ...
        upgrade_plugin_savepoint(true, 2026062510, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062511) {
        ...
        upgrade_plugin_savepoint(true, 2026062511, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062512) {
        // FOE-CATEGORY-SCOPE: No DB schema changes.
        // Section C replaced with category-scoped approach.
        upgrade_plugin_savepoint(true, 2026062512, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062513) {
        ...
        upgrade_plugin_savepoint(true, 2026062513, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062514) {
        ...
        upgrade_plugin_savepoint(true, 2026062514, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062515) {
        ...
        upgrade_plugin_savepoint(true, 2026062515, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062801) {
        ...
        upgrade_plugin_savepoint(true, 2026062801, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062802) {
        // NAT-RECONCILE-SIDEBAR: Added NAT Reconciliation Tool to lib.php sidebar nav. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026062802, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062803) {
        ...
        upgrade_plugin_savepoint(true, 2026062803, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062804) {
        ...
        upgrade_plugin_savepoint(true, 2026062804, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062805) {
        ...
        upgrade_plugin_savepoint(true, 2026062805, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062806) {
        ...
        upgrade_plugin_savepoint(true, 2026062806, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062807) {
        ...
        upgrade_plugin_savepoint(true, 2026062807, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062808) {
        ...
        upgrade_plugin_savepoint(true, 2026062808, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026062809) {
        ...
        upgrade_plugin_savepoint(true, 2026062809, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062810) {
        ...
        upgrade_plugin_savepoint(true, 2026062810, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062811) {
        ...
        upgrade_plugin_savepoint(true, 2026062811, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062812) {
        ...
        upgrade_plugin_savepoint(true, 2026062812, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062813) {
        ...
        upgrade_plugin_savepoint(true, 2026062813, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062814) {
        ...
        upgrade_plugin_savepoint(true, 2026062814, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026062815) {
        ...
        upgrade_plugin_savepoint(true, 2026062815, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070101) {
        ...
        upgrade_plugin_savepoint(true, 2026070101, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070102) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070102, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070103) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070103, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070104) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070104, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070105) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070105, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070106) {
        // Version 5.9.123: DOWNLOAD-FIX + STUDENT-DRILLDOWN
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070106, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070107) {
        // Version 5.9.124: RESTORE-COUNT-DIAGNOSTICS + SUSPENDED-FIX
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070107, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070108) {
        // Version 5.9.125: HISTORICAL-AVETMISS-CLASSIFIER
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070108, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070109) {
        // Version 5.9.126: COMPLAINT-STUDENT-REGRESSION-TEST-ENGINE
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070109, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070110) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070110, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070111) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070111, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070112) {
        // Version 5.9.129 — LOGSTORE-HISTORY: regression_test.php enriched with
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026070112, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070113) {
        // Version 5.9.130 — LOGSTORE-HISTORY-HARDENED: ChatGPT sign-off recommendations
        ...
        // No DB schema change.
        upgrade_plugin_savepoint(true, 2026070113, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070114) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070114, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070115) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070115, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070116) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070116, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070117) {
        ...
        upgrade_plugin_savepoint(true, 2026070117, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070118) {
        ...
        upgrade_plugin_savepoint(true, 2026070118, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070119) {
        // Version 5.9.136: Six critical bug fixes completing R1–R10 implementation.
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070119, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070120) {
        ...
        upgrade_plugin_savepoint(true, 2026070120, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070301) {
        ...
        upgrade_plugin_savepoint(true, 2026070301, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070302) {
        ...
        upgrade_plugin_savepoint(true, 2026070302, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070303) {
        ...
        upgrade_plugin_savepoint(true, 2026070303, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070304) {
        ...
        upgrade_plugin_savepoint(true, 2026070304, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070305) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026070305, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070306) {
        ...
        upgrade_plugin_savepoint(true, 2026070306, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070307) {
        // ARCHIVE-REVIEW-ONLY (v5.9.175): No DB schema changes — confidence routing fix only.
        upgrade_plugin_savepoint(true, 2026070307, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070308) {
        ...
        upgrade_plugin_savepoint(true, 2026070308, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070309) {
        ...
        upgrade_plugin_savepoint(true, 2026070309, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070310) {
        // VERSION-BUMP (v5.9.178): No DB schema changes — version increment to confirm ZIP integrity.
        upgrade_plugin_savepoint(true, 2026070310, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070311) {
        ...
        upgrade_plugin_savepoint(true, 2026070311, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070312) {
        ...
        upgrade_plugin_savepoint(true, 2026070312, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070313) {
        ...
        upgrade_plugin_savepoint(true, 2026070313, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070314) {
        ...
        upgrade_plugin_savepoint(true, 2026070314, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070315) {
        ...
        upgrade_plugin_savepoint(true, 2026070315, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070316) {
        ...
        upgrade_plugin_savepoint(true, 2026070316, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070317) {
        ...
        upgrade_plugin_savepoint(true, 2026070317, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070318) {
        ...
        upgrade_plugin_savepoint(true, 2026070318, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070319) {
        ...
        upgrade_plugin_savepoint(true, 2026070319, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070320) {
        ...
        upgrade_plugin_savepoint(true, 2026070320, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070321) {
        ...
        upgrade_plugin_savepoint(true, 2026070321, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070322) {
        ...
        upgrade_plugin_savepoint(true, 2026070322, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070323) {
        ...
        upgrade_plugin_savepoint(true, 2026070323, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070324) {
        // Version 5.9.192 CALL-SITE DEBUG: Create local_rtocompliance_qualdebug table.
        ...
        upgrade_plugin_savepoint(true, 2026070324, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070325) {
        // Version 5.9.193 QUAL-BRANCH-UNIQUE-95: New confidence tier — current + qual_branch +
        ...
        upgrade_plugin_savepoint(true, 2026070325, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070326) {
        // Version 5.9.194 SEM-MATCH-DOMINANT: Two-part fix for 192 ABC12345/ABC12345 false-positive
        ...
        upgrade_plugin_savepoint(true, 2026070326, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070327) {
        // Version 5.9.195 POOL-TRUNCATION-FIX: The normUnitAllCids merge guard in the ADD engine
        ...
        upgrade_plugin_savepoint(true, 2026070327, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070328) {
        ...
        upgrade_plugin_savepoint(true, 2026070328, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070329) {
        ...
        upgrade_plugin_savepoint(true, 2026070329, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070330) {
        ...
        upgrade_plugin_savepoint(true, 2026070330, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070331) {
        ...
        upgrade_plugin_savepoint(true, 2026070331, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070332) {
        ...
        upgrade_plugin_savepoint(true, 2026070332, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070333) {
        ...
        upgrade_plugin_savepoint(true, 2026070333, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070334) {
        ...
        upgrade_plugin_savepoint(true, 2026070334, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070335) {
        ...
        upgrade_plugin_savepoint(true, 2026070335, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070336) {
        ...
        upgrade_plugin_savepoint(true, 2026070336, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070337) {
        ...
        upgrade_plugin_savepoint(true, 2026070337, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070338) {
        ...
        upgrade_plugin_savepoint(true, 2026070338, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070339) {
        ...
        upgrade_plugin_savepoint(true, 2026070339, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070340) {
        ...
        upgrade_plugin_savepoint(true, 2026070340, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070341) {
        ...
        upgrade_plugin_savepoint(true, 2026070341, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070342) {
        ...
        upgrade_plugin_savepoint(true, 2026070342, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070343) {
        ...
        upgrade_plugin_savepoint(true, 2026070343, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070344) {
        ...
        upgrade_plugin_savepoint(true, 2026070344, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070345) {
        ...
        upgrade_plugin_savepoint(true, 2026070345, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070346) {
        ...
        upgrade_plugin_savepoint(true, 2026070346, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070347) {
        ...
        upgrade_plugin_savepoint(true, 2026070347, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070348) {
        ...
        upgrade_plugin_savepoint(true, 2026070348, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070349) {
        ...
        upgrade_plugin_savepoint(true, 2026070349, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070350) {
        ...
        upgrade_plugin_savepoint(true, 2026070350, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070351) {
        ...
        upgrade_plugin_savepoint(true, 2026070351, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070352) {
        ...
        upgrade_plugin_savepoint(true, 2026070352, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070353) {
        ...
        upgrade_plugin_savepoint(true, 2026070353, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070354) {
        ...
        upgrade_plugin_savepoint(true, 2026070354, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070355) {
        ...
        upgrade_plugin_savepoint(true, 2026070355, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070356) {
        ...
        upgrade_plugin_savepoint(true, 2026070356, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070357) {
        ...
        upgrade_plugin_savepoint(true, 2026070357, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070358) {
        ...
        upgrade_plugin_savepoint(true, 2026070358, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070359) {
        ...
        upgrade_plugin_savepoint(true, 2026070359, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026070360) {
        ...
        upgrade_plugin_savepoint(true, 2026070360, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026070361) {
        ...
        upgrade_plugin_savepoint(true, 2026070361, 'local', 'rtocompliance');
    }
    ...
    // Version 5.9.231 - FIX-XMLDB-DEFAULT: Removed empty-string DEFAULT from NOTNULL CHAR fields
    ...
    // Version 5.9.232 — INITIAL-COMPLETION-DATE-FIX: Certificate completion dates now
    ...
    if ($oldversion < 2026072301) {
        ...
        upgrade_plugin_savepoint(true, 2026072301, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072302) {
        ...
        upgrade_plugin_savepoint(true, 2026072302, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072303) {
        ...
        upgrade_plugin_savepoint(true, 2026072303, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072701) {
        ...
        upgrade_plugin_savepoint(true, 2026072701, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072702) {
        ...
        upgrade_plugin_savepoint(true, 2026072702, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072703) {
        ...
        upgrade_plugin_savepoint(true, 2026072703, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072704) {
        ...
        upgrade_plugin_savepoint(true, 2026072704, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072705) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072705, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072706) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072706, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072707) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072707, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072708) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072708, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072709) {
        ...
        upgrade_plugin_savepoint(true, 2026072709, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072710) {
        ...
        upgrade_plugin_savepoint(true, 2026072710, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072711) {
        ...
        upgrade_plugin_savepoint(true, 2026072711, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072712) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072712, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072713) {
        // Version 5.9.253 — QB-COURSE-REFRESH-FIX (27 Jul 2026):
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072713, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072714) {
        // Version 5.9.254 — CERT-ERRORS-FIX (27 Jul 2026):
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072714, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072715) {
        // Version 5.9.255 — QB-SEMESTER-SCOPE-FIX (27 Jul 2026):
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072715, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072716) {
        ...
        upgrade_plugin_savepoint(true, 2026072716, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072717) {
        ...
        upgrade_plugin_savepoint(true, 2026072717, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072718) {
        ...
        upgrade_plugin_savepoint(true, 2026072718, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072719) {
        ...
        upgrade_plugin_savepoint(true, 2026072719, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072720) {
        ...
        upgrade_plugin_savepoint(true, 2026072720, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072721) {
        ...
        upgrade_plugin_savepoint(true, 2026072721, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072722) {
        ...
        upgrade_plugin_savepoint(true, 2026072722, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072723) {
        ...
        upgrade_plugin_savepoint(true, 2026072723, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072724) {
        ...
        upgrade_plugin_savepoint(true, 2026072724, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072725) {
        ...
        upgrade_plugin_savepoint(true, 2026072725, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072726) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072726, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072727) {
        ...
        upgrade_plugin_savepoint(true, 2026072727, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072728) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072728, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072801) {
        ...
        // JS-only fix: qualbuilder_edit.js (src+build+min).  No PHP or DB changes.
        upgrade_plugin_savepoint(true, 2026072801, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072802) {
        ...
        upgrade_plugin_savepoint(true, 2026072802, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072803) {
        ...
        upgrade_plugin_savepoint(true, 2026072803, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072804) {
        ...
        upgrade_plugin_savepoint(true, 2026072804, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072805) {
        ...
        upgrade_plugin_savepoint(true, 2026072805, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072806) {
        ...
        //
        // No DB schema changes; JS-only release.
        upgrade_plugin_savepoint(true, 2026072806, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072807) {
        ...
        upgrade_plugin_savepoint(true, 2026072807, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072808) {
        ...
        upgrade_plugin_savepoint(true, 2026072808, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072809) {
        ...
        upgrade_plugin_savepoint(true, 2026072809, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072810) {
        ...
        upgrade_plugin_savepoint(true, 2026072810, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072811) {
        ...
        //
        // No PHP or DB schema changes.
        upgrade_plugin_savepoint(true, 2026072811, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072812) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072812, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072813) {
        ...
        upgrade_plugin_savepoint(true, 2026072813, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072814) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072814, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072815) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072815, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072816) {
        ...
        upgrade_plugin_savepoint(true, 2026072816, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072817) {
        ...
        upgrade_plugin_savepoint(true, 2026072817, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072818) {
        ...
        upgrade_plugin_savepoint(true, 2026072818, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072819) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072819, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072820) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072820, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072821) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072821, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072823) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072823, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072824) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072824, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072825) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072825, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072826) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072826, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072827) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072827, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072828) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072828, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072829) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072829, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072830) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072830, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072831) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072831, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072832) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072832, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072833) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072833, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072834) {
        ...
        upgrade_plugin_savepoint(true, 2026072834, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072835) {
        ...
        upgrade_plugin_savepoint(true, 2026072835, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072836) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072836, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072837) {
        ...
        upgrade_plugin_savepoint(true, 2026072837, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072838) {
        ...
        upgrade_plugin_savepoint(true, 2026072838, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072839) {
        ...
        //
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072839, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072840) {
        ...
        //
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072840, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072900) {
        ...
        //
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072900, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072901) {
        ...
        //
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072901, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072902) {
        ...
        //
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072902, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026072903) {
        ...
        //
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072903, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072904) {
        ...
        // No DB schema changes — upgrade step records the savepoint only.
        upgrade_plugin_savepoint(true, 2026072904, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072905) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072905, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026072906) {
        ...
        //
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026072906, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073000) {
        ...
        upgrade_plugin_savepoint(true, 2026073000, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073001) {
        ...
        upgrade_plugin_savepoint(true, 2026073001, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073002) {
        ...
        //
        // No schema changes.
        upgrade_plugin_savepoint(true, 2026073002, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073003) {
        ...
        upgrade_plugin_savepoint(true, 2026073003, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073004) {
        ...
        upgrade_plugin_savepoint(true, 2026073004, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073005) {
        ...
        upgrade_plugin_savepoint(true, 2026073005, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073006) {
        ...
        upgrade_plugin_savepoint(true, 2026073006, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073007) {
        ...
        upgrade_plugin_savepoint(true, 2026073007, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073008) {
        ...
        upgrade_plugin_savepoint(true, 2026073008, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073009) {
        ...
        upgrade_plugin_savepoint(true, 2026073009, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073010) {
        ...
        upgrade_plugin_savepoint(true, 2026073010, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073011) {
        ...
        upgrade_plugin_savepoint(true, 2026073011, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073012) {
        ...
        upgrade_plugin_savepoint(true, 2026073012, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073013) {
        ...
        upgrade_plugin_savepoint(true, 2026073013, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073014) {
        ...
        upgrade_plugin_savepoint(true, 2026073014, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073015) {
        ...
        upgrade_plugin_savepoint(true, 2026073015, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073016) {
        ...
        upgrade_plugin_savepoint(true, 2026073016, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073017) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026073017, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073018) {
        ...
        upgrade_plugin_savepoint(true, 2026073018, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073019) {
        // QUAL-CERT-HUB (v5.9.339): new qual_cert_hub.php page for unified
        // qualification certificate management. No DB schema changes.
        upgrade_plugin_savepoint(true, 2026073019, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073020) {
        ...
        upgrade_plugin_savepoint(true, 2026073020, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073021) {
        ...
        upgrade_plugin_savepoint(true, 2026073021, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073022) {
        ...
        upgrade_plugin_savepoint(true, 2026073022, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073023) {
        ...
        upgrade_plugin_savepoint(true, 2026073023, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073024) {
        ...
        upgrade_plugin_savepoint(true, 2026073024, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073025) {
        ...
        upgrade_plugin_savepoint(true, 2026073025, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073026) {
        ...
        upgrade_plugin_savepoint(true, 2026073026, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073027) {
        // TASK-52 (v5.9.350): Missing-units panel on Partially Complete tab — no DB schema changes.
        upgrade_plugin_savepoint(true, 2026073027, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073028) {
        ...
        upgrade_plugin_savepoint(true, 2026073028, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073029) {
        ...
        upgrade_plugin_savepoint(true, 2026073029, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073030) {
        ...
        upgrade_plugin_savepoint(true, 2026073030, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073100) {
        ...
        upgrade_plugin_savepoint(true, 2026073100, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073101) {
        ...
        upgrade_plugin_savepoint(true, 2026073101, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073102) {
        ...
        upgrade_plugin_savepoint(true, 2026073102, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073103) {
        // Version 5.9.365: Cert-issuance pipeline fixes (cert_template, cert_template_renderer,
        ...
        upgrade_plugin_savepoint(true, 2026073103, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073104) {
        // Version 5.9.368 TIMECOMPLETED-COLUMN: programmatic_issue_cert() has always set
        ...
        upgrade_plugin_savepoint(true, 2026073104, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073105) {
        // Version 5.9.387 ASQA PRACTICE-GUIDE AUDIT REMEDIATION — schema for the P1 fixes.
        ...
        upgrade_plugin_savepoint(true, 2026073105, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073106) {
        // Version 5.9.389 CERTIFICATE-INTEGRITY (audit F1): snapshot the RTO identity settings
        ...
        upgrade_plugin_savepoint(true, 2026073106, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073107) {
        // Version 5.9.396 ADDRESS PROPAGATION: the NAT00085 street address (postcode, building,
        ...
        upgrade_plugin_savepoint(true, 2026073107, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073108) {
        // Version 5.9.401 COMPLAINTS/APPEALS PROCEDURAL FAIRNESS (Std 2.7/2.8).
        $table = new xmldb_table('local_rtocompliance_complaints');
        foreach ([
        ...
        upgrade_plugin_savepoint(true, 2026073108, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026073109) {
        // Version 5.9.407 SANITISE EXISTING SAVED CERT TEMPLATES.
        //
            ...
            if ($dbman->table_exists(new xmldb_table('local_rtocompliance_certtmpl'))) {
                ...
                foreach ($rs as $tpl) {
                    ...
                    if ($changed) {
                        $design['fields'] = $newfields;
                        $DB->set_field(
                            'local_rtocompliance_certtmpl', 'designjson',
        ...
        upgrade_plugin_savepoint(true, 2026073109, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073110) {
        ...
        upgrade_plugin_savepoint(true, 2026073110, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073111) {
        ...
        $field = new xmldb_field(
            'outcomecommunicated', XMLDB_TYPE_INTEGER, '1', null,
        ...
        $field = new xmldb_field(
            'outcomecommunicateddate', XMLDB_TYPE_INTEGER, '10', null,
        ...
        $field = new xmldb_field(
            'outcomecommunicatedmethod', XMLDB_TYPE_CHAR, '100', null,
            null, null, null, 'outcomecommunicateddate');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_plugin_savepoint(true, 2026073111, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026073112) {
        ...
        $field = new xmldb_field(
            'supersededunitcode', XMLDB_TYPE_CHAR, '20', null,
        ...
        $field = new xmldb_field(
            'unitequivalence', XMLDB_TYPE_CHAR, '20', null,
        ...
        $field = new xmldb_field(
            'evidencematrix', XMLDB_TYPE_TEXT, null, null,
            null, null, null, 'unitequivalence');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_plugin_savepoint(true, 2026073112, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026080400) {
        ...
        upgrade_plugin_savepoint(true, 2026080400, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080401) {
        // Version 6.0.0 — force a visible DB upgrade and guarantee the AVETMISS round-trip columns.
        ...
        upgrade_plugin_savepoint(true, 2026080401, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080402) {
        // Version 6.0.1 — no schema change; this savepoint simply advances the recorded database
        // version so the new visible version badge reflects a genuine, committed DB upgrade.
        upgrade_plugin_savepoint(true, 2026080402, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080403) {
        // Version 6.0.2 — no schema change (Semester Builder unit-set inference is scan-time logic only).
        upgrade_plugin_savepoint(true, 2026080403, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080404) {
        // Version 6.0.3 — no schema change (Semester Builder category/course links are display-only).
        upgrade_plugin_savepoint(true, 2026080404, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080405) {
        // Version 6.0.4 — no schema change (Semester Builder parent-category grouping is display-only).
        upgrade_plugin_savepoint(true, 2026080405, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080406) {
        // Version 6.0.5 — no schema change (Semester Builder width + display tidy only).
        upgrade_plugin_savepoint(true, 2026080406, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080407) {
        // Version 6.0.6 — no schema change (sticky table headers are CSS-only).
        upgrade_plugin_savepoint(true, 2026080407, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080408) {
        // Version 6.0.7 — no schema change (Build Course Map button reuses existing seeding logic).
        upgrade_plugin_savepoint(true, 2026080408, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080409) {
        // Version 6.0.8 — no schema change (help/assistant content refresh only).
        upgrade_plugin_savepoint(true, 2026080409, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080410) {
        // Version 6.0.9 — no schema change (help-accuracy fixes + assistant KB enrichment only).
        upgrade_plugin_savepoint(true, 2026080410, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080411) {
        // Version 6.1.0 — no schema change (State Funding label/help corrections + assistant funding KB).
        upgrade_plugin_savepoint(true, 2026080411, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080412) {
        // Version 6.1.1 — no schema change (cert cost confirmations + assistant page links).
        upgrade_plugin_savepoint(true, 2026080412, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080413) {
        // Version 6.1.2 — no schema change (USI status page admin_externalpage_setup fix).
        upgrade_plugin_savepoint(true, 2026080413, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080414) {
        // Version 6.1.3 — no schema change (plugin-wide width CSS + My Documents card styling).
        upgrade_plugin_savepoint(true, 2026080414, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080415) {
        // Version 6.1.4 — no schema change (cert hub variant column/card + state funding link).
        upgrade_plugin_savepoint(true, 2026080415, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080416) {
        // Version 6.2.0 — no schema change (FAQ page + plugin-wide tooltips/cards + filter fix).
        upgrade_plugin_savepoint(true, 2026080416, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080417) {
        // Version 6.2.1 — no schema change (simple-language help what/why + pipeline fixes).
        upgrade_plugin_savepoint(true, 2026080417, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080418) {
        // Version 6.2.2 — no schema change (simple-language how/features + sticky bar + pill tooltips).
        upgrade_plugin_savepoint(true, 2026080418, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080419) {
        // Version 6.2.3 — no schema change (help pills repointed to lms-labs guide mirror).
        upgrade_plugin_savepoint(true, 2026080419, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080420) {
        // Version 6.2.4 — no schema change (changelog wording fix for the style scanner).
        upgrade_plugin_savepoint(true, 2026080420, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080421) {
        // Version 6.2.5 — no schema change (plugin-wide pill/badge/stat-tile tooltips + capability checks on 5 admin pages).
        upgrade_plugin_savepoint(true, 2026080421, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080422) {
        // Version 6.2.6 — no schema change (cert editor nested-form fix, min-font-12 in editor,
        // sample checkbox removed, ASQA wording fix, cert orientation filter setting).
        upgrade_plugin_savepoint(true, 2026080422, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080423) {
        // Version 6.2.7 — no schema change (AI certificate auto-design: new editor button + proxy
        // endpoint calling the platform vision endpoint).
        upgrade_plugin_savepoint(true, 2026080423, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080424) {
        // Version 6.2.8 — no schema change (USI-verified accuracy: centralised STATUS_VERIFIED===1
        // semantics across exports/displays/issuance gates; certificate header theme colour).
        upgrade_plugin_savepoint(true, 2026080424, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080425) {
        // Version 6.2.9 — no schema change (Record of Results 'Results' column via col3mode + duplicate
        // header removal; AVETMISS terminology on the roster CSV export).
        upgrade_plugin_savepoint(true, 2026080425, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080426) {
        // Version 6.2.10 — no schema change (branding: AI Grader / Essay Grader AI → LMS-Labs.com).
        upgrade_plugin_savepoint(true, 2026080426, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080427) {
        // Version 6.2.11 — no schema change (outline-button hover contrast + 10mm minimum text-field height).
        upgrade_plugin_savepoint(true, 2026080427, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080428) {
        // Version 6.2.12 — no schema change (auto-design curl/filelib fix; per-cert-type background
        // default fix; certificate-editor button hover contrast).
        upgrade_plugin_savepoint(true, 2026080428, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080429) {
        // Version 6.2.13 — no schema change (auto-design replaces the layout instead of appending).
        upgrade_plugin_savepoint(true, 2026080429, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080430) {
        // Version 6.2.14 — no schema change (bigger certificate-editor Properties panel: capped
        // validator + wider right column).
        upgrade_plugin_savepoint(true, 2026080430, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080431) {
        // Version 6.2.15 — no schema change (live preview honours per-cert-type background/org-seal
        // applies-to config, matching the issued certificate).
        upgrade_plugin_savepoint(true, 2026080431, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080432) {
        // Version 6.2.16 — no schema change (live ASQA re-validation: new read-only AJAX endpoint
        ...
        upgrade_plugin_savepoint(true, 2026080432, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080433) {
        // Version 6.2.17 — no schema change (USI Verification page gains a student dashboard:
        ...
        upgrade_plugin_savepoint(true, 2026080433, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080434) {
        // Version 6.2.18 — no schema change (self-service USI credential upload on the USI
        ...
        upgrade_plugin_savepoint(true, 2026080434, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080435) {
        // Version 6.2.19 — no schema change (self-service upload now persists the platform-
        ...
        upgrade_plugin_savepoint(true, 2026080435, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080436) {
        // Version 6.2.20 — no schema change (credential upload panel now accepts the ABR
        ...
        upgrade_plugin_savepoint(true, 2026080436, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026080437) {
        // Version 6.2.21 — no schema change (Student Records missing-DOB view gains a simple
        ...
        upgrade_plugin_savepoint(true, 2026080437, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026080500) {
        ...
        upgrade_plugin_savepoint(true, 2026080500, 'local', 'rtocompliance');
    }
    ...
    if ($oldversion < 2026080501) {
        ...
        upgrade_plugin_savepoint(true, 2026080501, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026081400) {
        // AVETMISS-PROFILE-GATE + USI-TABLE-REDESIGN (v6.3.0 / 14 Aug 2026):
        //
        // 1. PROFILE GATE. A student in nationally recognised training who is
        //    missing mandatory AVETMISS data is now HELD at My AVETMISS Profile
        //    until they complete it. v5.9.314 only nudged them once, on the first
        //    page after login, and they could click straight past it — so students
        //    trained for months with no date of birth or USI on file, which blocks
        //    USI verification, certificate issuance and the NAT00080 submission.
        //
        //    - lib.php: local_rtocompliance_profile_gate_check() redirects on every
        //      page build; the allowlist covers login/logout, password reset, site
        //      policies, Moodle's own required-profile form, admin pages, file
        //      serving, AJAX and web services so nobody can be locked out.
        //    - lib.php: one shared field definition (avetmiss_mandatory_fields,
        //      avetmiss_value_missing, get_missing_avetmiss_fields,
        //      calculate_profilecomplete) now used by the gate, my_profile.php and
        //      student_profile.php — the three paths can no longer disagree about
        //      what "complete" means.
        //    - lib.php: user_requires_avetmiss() also matches students holding
        //      active local_rtocompliance_enrolments rows (the Qual Builder path),
        //      not only courses carrying the legacy nationallyrecognised flag.
        //    - classes/hook/before_standard_head_html_generation.php: backstop for
        //      page layouts that never build the global navigation.
        //    - db/access.php: new local/rtocompliance:bypassprofilegate capability.
        //    - my_profile.php + student_profile_form.php: locked mode — outstanding
        //      fields listed, mandatory fields enforced in validation, Cancel
        //      removed, and the student returned to where they were headed.
        //
        // 2. USI VERIFICATION TABLE. Stat cards now recount inside the current
        //    category/course/search scope (they were whole-of-site, and "Not yet
        //    verified" wrongly included every student with no USI at all), sit
        //    directly above the filters, and act as one-click filters. Added a
        //    Clear filters button, active-filter chips, a 25/50/100/200 per-page
        //    selector, sortable columns, a PDF export of the filtered view, and a
        //    Course dropdown that genuinely narrows to the selected Category.
        //
        // Defaults are written here so the gate is live immediately after upgrade
        // without an administrator having to visit the settings page first.
        if (get_config('local_rtocompliance', 'enforceprofile') === false) {
            set_config('enforceprofile', 1, 'local_rtocompliance');
        }
        // Note the USI is deliberately NOT in the default lock list — a student
        // cannot obtain one on demand, so requiring it to reach the site would hold
        // them with no way to comply. It stays part of the completeness definition
        // (and of certificate issuance), and an administrator can tick it on.
        if (get_config('local_rtocompliance', 'mandatoryprofilefields') === false) {
            set_config(
                'mandatoryprofilefields',
                    'dateofbirth,sex,postcode,statecode,suburb,indigenousstatus,'
                    . 'countryofbirth,languageathome,labourforcestatus,highestschoollevel',
                'local_rtocompliance');
        }

        // 3. SAVEPOINT RENUMBERING SAFETY NET. All 792 savepoints in this file were
        //    renumbered from 13-digit to correct 10-digit Moodle versions in v6.3.0.
        //    Historical steps are therefore now genuinely skipped on an existing site
        //    (previously $oldversion never matched, so every step re-ran and the schema
        //    was continually re-asserted as a side effect). Reconcile the live database
        //    against install.xml once, here, so nothing can be left behind by that
        //    change. Additive only — creates missing tables, adds missing columns,
        //    never drops or retypes anything.
        $syncdbman = $DB->get_manager();
        $sync = local_rtocompliance_upgrade_sync_schema($syncdbman);
        if (!empty($sync['tables'])) {
            upgrade_log(
                UPGRADE_LOG_NORMAL, 'local_rtocompliance',
                'Schema sync created table(s): ' . implode(', ', $sync['tables']));
        }
        if (!empty($sync['fields'])) {
            upgrade_log(
                UPGRADE_LOG_NORMAL, 'local_rtocompliance',
                'Schema sync added column(s): ' . implode(', ', $sync['fields']));
        }
        if (!empty($sync['skipped'])) {
            upgrade_log(
                UPGRADE_LOG_NOTICE, 'local_rtocompliance',
                'Schema sync skipped (needs a human): ' . implode('; ', $sync['skipped']));
        }

        // No DB schema changes of its own — the savepoint plus the reconciliation above.
        upgrade_plugin_savepoint(true, 2026081400, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026081700) {
        // Version 6.3.10 PRE-1970 DATE OF BIRTH FIX. A DOB before 1 Jan 1970 is a
    ...
    if ($oldversion < 2026081901) {
        // Version 6.3.13 CERTIFICATE GENERATION USI PREFLIGHT. No schema change: the fix is
    ...
    if ($oldversion < 2026081902) {
        // Version 6.3.14 ASSISTANT KNOWLEDGE SOURCES. No schema change: the assistant gains a
    ...
    if ($oldversion < 2026081903) {
        // Version 6.3.15 ASSISTANT REQUEST PARAMETER TIGHTENING. No schema change and no stored data
    ...
    if ($oldversion < 2026082000) {
        // Version 6.3.16 — code-only release. lib.php prints the source unit on transcripts
    ...
    if ($oldversion < 2026082001) {
        // Version 6.3.17 — code-only release. Multi-page Record of Results no longer strands
    ...
    if ($oldversion < 2026082002) {
        // Version 6.3.18 — code-only. Corrects the v6.3.17 approach to the missing page-1
    ...
    if ($oldversion < 2026082003) {
        // Version 6.3.19 — USI EXEMPTION FOR OFFSHORE INTERNATIONAL STUDENTS.
    ...
    if ($oldversion < 2026082104) {
        // Version 6.3.20 — no schema change. Certificate table column headings became configurable
        // (site-wide in Certificate Settings, and per table field in the template editor), and
        // the stale plain-text caption row above a self-heading table is now stripped at render
        // time so existing Record of Results templates stop printing two header rows.
        upgrade_plugin_savepoint(true, 2026082104, 'local', 'rtocompliance');
    }

    if ($oldversion < 2026082105) {
        // Version 6.3.21 — no schema change, and no behaviour change either: a coding-style and
