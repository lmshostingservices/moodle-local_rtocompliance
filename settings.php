...
if ($canviewfull) {
    ...
    if ($ADMIN->fulltree && isset($settings)) {
        ...
        // CERT-TABLE-HEADINGS (v6.3.20): the column heading wording on the certificate
        // tables. Defaults follow the ASQA sample forms; an RTO that uses different
        // terminology overrides any of them here. Leave a field empty to keep the default.
        // A single certificate template can override again, per table, in the template
        // editor's Field Properties panel.
        $settings->add(
            new admin_setting_heading(
                'local_rtocompliance/certtableheadings',
                get_string('certtableheadings', 'local_rtocompliance'),
                get_string('certtableheadings_desc', 'local_rtocompliance')
        ));
        foreach (local_rtocompliance_cert_table_heading_defaults() as $rtoc_slot => $rtoc_default) {
            $settings->add(
                new admin_setting_configtext(
                    'local_rtocompliance/certtablehead_' . $rtoc_slot,
                    get_string('certtablehead_' . $rtoc_slot, 'local_rtocompliance'),
                    get_string('certtablehead_desc', 'local_rtocompliance', $rtoc_default),
                    '',
                    PARAM_TEXT,
                    40
            ));
    ...
    if ($ADMIN->fulltree && isset($settings)) {
        ...
        // Version 6.3.14: let the assistant see this site's own data. Without it the assistant can
    ...
    if ($ADMIN->fulltree && isset($settings)) {
        ...
        // Version 5.9.361: per-type starting number — lets an RTO begin a stream at, e.g.,
